    ...
    window.fetch = async function(input, init) {
				...
				if (isPayPalOrder) {
					...
					await fetch((window.myRecaptchaVars || {}).AJAX_URL_I13 || '/wp-admin/admin-ajax.php', {
					  ...
					  body: new URLSearchParams({ action: 'save_recaptcha_tokenv2', 'g-recaptcha-response-v2':window.myRecaptchaTokenI13,'nonce':NONCE })
					});
                ...
                if (!clonedBody.extensions) {
                 ...
                 if (!clonedBody.extensions['i13-recaptcha-checkout-block']) {
                    clonedBody.extensions['i13-recaptcha-checkout-block'] = {
                        'g-recaptcha-response-v2': window.myRecaptchaTokenI13 || ''
                    };
                }
...
function renderNormalRecaptchaWhenReady() {
	...
	const tryRender = () => {
		...
		if (
			...
			/*console.log('✅ grecaptcha.render is ready, rendering now');*/
			captchaWidget=window.grecaptcha.render('recaptcha-block', {
				sitekey: SITE_KEY,
...
function renderEnterpriseRecaptchaWhenReady() {
	const maxAttempts = 20;
	let attempts = 0;
	...
	const tryRender = () => {
		...
		if (
			...
			/*console.log('✅ grecaptcha.render is ready, rendering now');*/
			captchaWidget=window.grecaptcha.enterprise.render('recaptcha-block', {
				sitekey: SITE_KEY,
				callback: function (response) {
					window.myRecaptchaTokenI13 = response;
					removeRecaptchaOverlay();
...
function initCartCaptcha() {
	const checkInterval = setInterval(() => {
		...
		if (expressContainer && !document.getElementById('recaptcha-block')) {
			...
			captchaRef.id = 'ref-captcha-v2-i13';
			...
			if(IS_ENTERPRISE_CAPTCHA=='1'){
				...
				renderEnterpriseRecaptchaWhenReady();

				document.getElementById('ref-captcha-v2-i13').addEventListener('click', function () {
				window.grecaptcha.enterprise.reset(captchaWidget);
				window.myRecaptchaTokenI13=null;
			});
			}
			else{
