...
?><script type="text/javascript">
    ...
    jQuery(document).ready(function(){
        ...
        jQuery(document).on('click', "span.deactivate a", function(e){
            if ( jQuery(this).closest('tr').attr('data-plugin') === '<?php echo $plugin_file; ?>' ) {
                e.preventDefault();
                let tb_id = '#TB_inline?inlineId=<?php echo $thickbox_id; ?>';
                tb_show('<?php _e('Plugin deactivation','wp-wpdesk-tracker-deactivation'); ?>', tb_id);
                resize_deactivation_tb_window();
            }
        });

        jQuery(document).on( 'click', '.<?php echo $thickbox_id; ?> .tracker-button-close', function(e) {
            e.preventDefault();
            tb_remove();
        });

        jQuery(document).on( 'click', '.<?php echo $thickbox_id; ?> .allow-deactivate', function(e) {
            e.preventDefault();
            let url = jQuery("tr[data-plugin='<?php echo $plugin_file; ?>']").find('span.deactivate a').attr('href');
            let reason = jQuery('.<?php echo $thickbox_id; ?> input[name=selected-reason]:checked').val();
            let additional_info = jQuery('.<?php echo $thickbox_id; ?> input[name=selected-reason]:checked').closest('li').find('input.additional-info').val();
        ...
        jQuery(document).on( 'click', '.<?php echo $thickbox_id; ?> input[type=radio]', function(){
            ...
            resize_deactivation_tb_window();
            jQuery('.<?php echo $thickbox_id; ?> .button-deactivate').html( '<?php _e( 'Submit &amp; Deactivate', 'wp-wpdesk-tracker-deactivation' ); ?>' );
        });
    });
...
<style>
    ...
    .<?php echo $thickbox_id; ?> input[type=text] {
        margin-left: 25px;
        width: 90%;
    }
</style>
