 ...
 * .
 */
class PluginLinks implements \FSProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    {
        $extensions_link = \add_query_arg('page', \FSProVendor\Octolize\ShippingExtensions\Page::MENU_SLUG, \admin_url('admin.php'));
        $extensions_link = \add_query_arg(self::PLUGIN_LINKS_PAGE, '', $extensions_link);
        return '<a href="' . \esc_url($extensions_link) . '" style="color:#917dff;font-weight:bold;">' . \_x('Extensions', 'Link on plugin list page', 'flexible-shipping-pro') . '</a>';
    }
}
