...
use FSProVendor\WPDesk_Flexible_Shipping;
/**
...
class BlockSettings
{
     ...
     */
    public function __construct(\FSProVendor\WPDesk\FS\Compatibility\PluginCompatibilityChecker $plugin_compatibility_checker)
    {
    ...
    private function get_general_message()
    {
        ...
        return \sprintf(\__('In order to prevent any further issues with the plugin configuration or its proper functioning, before saving the changes please update the following: %s.', 'flexible-shipping-pro'), \sprintf('<strong>%s</strong>', $plugins));
    }
    /**
     * @return string
     */
    ...
    {
        return \__('Go to the plugins list &rarr;', 'flexible-shipping-pro');
    }
    /**
     * @return string
     */
