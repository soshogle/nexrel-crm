 ...
 */
class LicensePage implements \FSProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    public function __construct(\FSProVendor\WPDesk\License\InstalledPlugins $plugin_database)
    {
    ...
    private function execute_plugin_action($plugin, $action)
    {
        ...
        if (isset($plugin_info)) {
            (new \FSProVendor\WPDesk\License\Page\LicensePageActions())->create_action($action)->execute($plugin_info);
        }
    }
    /**
