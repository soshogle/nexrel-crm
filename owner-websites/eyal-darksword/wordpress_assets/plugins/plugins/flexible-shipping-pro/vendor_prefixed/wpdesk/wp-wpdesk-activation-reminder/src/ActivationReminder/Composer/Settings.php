...
class Settings
{
    ...
    public static function validate_settings($settings)
    {
        if (!isset($settings[self::ACTIVATION_REMINDER])) {
            throw new \FSProVendor\WPDesk\ActivationReminder\Composer\InvalidSettingValue(self::ACTIVATION_REMINDER, null);
        }
        if (!\is_array($settings[self::ACTIVATION_REMINDER])) {
            throw new \FSProVendor\WPDesk\ActivationReminder\Composer\InvalidSettingValue(self::ACTIVATION_REMINDER, 'should be array');
        }
        $settings_fields = [self::PLUGIN_TITLE, self::PLUGIN_DIR, self::LOGO_URL, self::BUY_PLUGIN_URL, self::HOW_TO_ACTIVATE_LINK];
        foreach ($settings_fields as $field) {
            if (!isset($settings[self::ACTIVATION_REMINDER][$field])) {
                throw new \FSProVendor\WPDesk\ActivationReminder\Composer\InvalidSettingValue($field, null);
            }
        }
    }
    /**
    ...
    public static function create_from_composer_settings(array $settings)
    {
        ...
        return new self($settings[self::ACTIVATION_REMINDER][self::PLUGIN_TITLE], $settings[self::ACTIVATION_REMINDER][self::PLUGIN_DIR], $settings[self::ACTIVATION_REMINDER][self::LOGO_URL], $settings[self::ACTIVATION_REMINDER][self::BUY_PLUGIN_URL], $settings[self::ACTIVATION_REMINDER][self::HOW_TO_ACTIVATE_LINK]);
    }
}
