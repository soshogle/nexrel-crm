	...
	public function add_tracking_info_to_csv_export_column_data( $order_data, $order, $csv_generator ) {
		...
		if ( count( $tracking_items ) > 0 ) {
			foreach ( $tracking_items as $item ) {
				$pipe                 = null;
				$item['date_shipped'] = $item['formatted_date_shipped_ymd'];
				unset( $item['formatted_date_shipped_ymd'] );
				unset( $item['formatted_date_shipped_wc'] );
				unset( $item['formatted_date_shipped_i18n'] );
		...
		// `wc_customer_order_csv_export()` is a third party function.
		if ( function_exists( 'wc_customer_order_csv_export' ) && version_compare( wc_customer_order_csv_export()->get_version(), '4.0.0', '<' ) ) {
