...
class WC_Shipment_Tracking_Actions {
	...
	public function get_shipment_tracking_column( $order_id ) {
				...
				foreach ( $tracking_items as $index => $tracking_item ) {
					...
					if ( 0 === $index ) {
						printf(
							...
							esc_url( $formatted['formatted_tracking_link'] ),
							esc_html( $tracking_item['tracking_number'] ),
							// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped --- $number_of_tracking_items is int|null.
							$number_of_tracking_items - 1
						);
					} else {
						printf(
		 ...
		 *
		 * @param string $display_text Column display text.
		 * @param int    $order_id Order ID.
	...
	public function display_html_tracking_item_for_meta_box( $order_id, $item ) {
			...
			// translators: %s is formatted date shipped value.
			$date_shipped_text = ! empty( $formatted['formatted_date_shipped_i18n'] ) ? sprintf( __( 'Shipped on %s', 'woocommerce-shipment-tracking' ), $formatted['formatted_date_shipped_i18n'] ) : __( 'No shipping date', 'woocommerce-shipment-tracking' );
		?>
			<div class="tracking-item" id="tracking-item-<?php echo esc_attr( $item['tracking_id'] ); ?>">
				...
				<p class="meta">
					...
					<?php echo esc_html( $date_shipped_text ); ?>
	...
	public function meta_box( $post_or_order_object ) {
		...
		if ( ! $selected_provider ) {
			 ...
			 *
			 * @param string $default_provider_name provider name.
			 *
			 * @since 1.4.0
			 */
	...
	public function display_tracking_info( $order_id ) {
		...
		wc_get_template(
			'myaccount/wcst-tracking-info.php',
			array(
				'order'          => $order,
	...
	public function get_excluded_email_classes() {
		 ...
		 *
		 * @param array $email_clases List of email classes.
		 *
		 * @since 2.1.0
		 */
		return apply_filters(
	...
	public function get_formatted_tracking_item( $order_id, $tracking_item ) {
		...
		$formatted['formatted_date_shipped_ymd']  = '';
		$formatted['formatted_date_shipped_wc']   = '';
		$formatted['formatted_date_shipped_i18n'] = '';
		...
		if ( ! empty( $tracking_item['date_shipped'] ) && is_numeric( $tracking_item['date_shipped'] ) ) {
			$formatted['formatted_date_shipped_ymd']  = gmdate( 'Y-m-d', $tracking_item['date_shipped'] );
			$formatted['formatted_date_shipped_wc']   = gmdate( wc_date_format(), $tracking_item['date_shipped'] );
			$formatted['formatted_date_shipped_i18n'] = date_i18n( wc_date_format(), $tracking_item['date_shipped'] );
		}
		 ...
		 * @param WC_Shipment_Tracking_Actions $wcst_actions WC_Shipment_Tracking_Actions object.
		 *
		 * @since 2.1.0
		 */
