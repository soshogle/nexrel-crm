jQuery(document).ready(function ($) {
    ...
    function insert_currency() {
        ...
        jQuery('.wmc-add-currency').on('click', function () {
            ...
            new_row.find('input[name="woo_multi_currency_params[currency_rate][]"]').removeAttr('readonly');
            new_row.find('input[name="woo_multi_currency_params[currency_rate_fee][]"]').removeAttr('readonly');
    ...
    $('select[name="woo_multi_currency_params[auto_detect]"]').on('change', function () {
        switch (true) {
            case $(this).val() === '2':
                ...
                $('.wmc-initial-currency-row').hide();
                $('.wmc-user-login-switch').hide();
                break;
            case $(this).val() === '1':
                $('.wmc-order-preview-row').hide();
                $('.wmc-initial-currency-row').hide();
                $('.wmc-user-login-switch').show();
                break;
            case $(this).val() === '0':
                $('.wmc-order-preview-row').hide();
                $('.wmc-initial-currency-row').hide();
    ...
    $('select[name="woo_multi_currency_params[geo_api]"]').on('change', function () {
        switch (true) {
            case $(this).val() === '1':
                $('.wmc-external-geo-api-key-row').show();
                break;
            default:
                $('.wmc-external-geo-api-key-row').hide();
        }
    });
    ...
    $('select[name="woo_multi_currency_params[finance_api]"]').on('change', function () {
        let val = $(this).val();
        //wmc-open-exchange-rates
        switch (val) {
            case '4':
                $('.wmc-wise-api-token').show();
                $('.wmc-open-exchange-rates').hide();
                $('.wmc-exchangeratesapi-rates').hide();
                $('.wmc-currencyapi-rates').hide();
                break;
            case '6':
                $('.wmc-wise-api-token').hide();
                $('.wmc-open-exchange-rates').show();
                $('.wmc-exchangeratesapi-rates').hide();
                $('.wmc-currencyapi-rates').hide();
                break;
            case '7':
                $('.wmc-wise-api-token').hide();
                $('.wmc-open-exchange-rates').hide();
                $('.wmc-exchangeratesapi-rates').show();
                $('.wmc-currencyapi-rates').hide();
                break;
            case '8':
                $('.wmc-wise-api-token').hide();
    ...
    $('select[name="woo_multi_currency_params[sidebar_layout]"]').on('change', function () {
        let val = $(this).val();
        if ('single' === val) {
            $('.wmc-name-background-color').fadeIn(200);
            $('.wmc-des-single-slide').fadeIn(200);
        } else {
            $('.wmc-name-background-color').fadeOut(200);
            $('.wmc-des-single-slide').fadeOut(200);
        }
        if ('hover_detail' === val) {
            $('.wmc-des-hover-detail').fadeIn(200);
        } else {
            $('.wmc-des-hover-detail').fadeOut(200);
        }
    }).trigger('change');
});
