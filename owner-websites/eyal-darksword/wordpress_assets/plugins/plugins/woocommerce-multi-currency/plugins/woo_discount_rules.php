...
class WOOMULTI_CURRENCY_Plugin_Woo_Discount_Rules {
	...
	public function __construct() {
		...
		if ( $this->settings->get_enable() && is_plugin_active( 'woo-discount-rules/woo-discount-rules.php' ) ) {
			...
			if ( defined( 'WDR_VERSION' ) && version_compare( WDR_VERSION, '2.6.5', '<' ) ) {
...
//			) ,10,2);
			...
			//subtotal rule type
			add_filter( 'advanced_woo_discount_rules_get_cart_subtotal', array(
				$this,
				'advanced_woo_discount_rules_get_cart_subtotal'
			) );
			add_filter( 'advanced_woo_discount_rules_line_item_subtotal', array(
				$this,
				'advanced_woo_discount_rules_line_item_subtotal'
			), 10, 3 );
		}
	}
	...
	public function advanced_woo_discount_rules_get_cart_subtotal( $subtotal ) {
	    if ( ! $subtotal || $this->settings->check_fixed_price() ) {
	        return $subtotal;
        }

		return wmc_revert_price( $subtotal );
	}

	public function advanced_woo_discount_rules_line_item_subtotal( $line_subtotal, $cart_item, $tax_display_type ) {
	    if ( ! $line_subtotal || $this->settings->check_fixed_price() ) {
	        return $line_subtotal;
        }

		return wmc_revert_price( $line_subtotal );
	}
