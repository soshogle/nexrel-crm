...
class WOOMULTI_CURRENCY_Plugin_Yith_Frequently_Bought_Together {
	...
	public function yith_wfbt_total_html( $total_html, $total, $total_discount, $product_id, $products ) {
		$total_html = ! is_null( $total_discount ) ? do_shortcode( "[woo_multi_currency_exchange price=" . esc_attr( wmc_revert_price( $total_discount ) ) . " original_price=" . esc_attr( wmc_revert_price( $total ) ) . "]" ) : do_shortcode( "[woo_multi_currency_exchange price=" . esc_attr( wmc_revert_price( $total ) ) . "]" );
	...
	public function yith_wfbt_discount_html( $discount, $product_id, $products ) {
		...
		return do_shortcode( "[woo_multi_currency_exchange price=" . esc_attr( wmc_revert_price( $discount ) ) . "]" );
	}
}