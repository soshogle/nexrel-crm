...
//						'toret_ppl_returnimport',
			...
			$elex_dhl_settings = get_option('woocommerce_wf_dhl_shipping_settings');
			if ( $elex_dhl_settings && is_array( $elex_dhl_settings ) && isset( $elex_dhl_settings['dhl_currency_type'] ) &&
			     $currency == $elex_dhl_settings['dhl_currency_type'] ) {
				array_push( $exclude_convert_method, 'wf_dhl_shipping' );
			}
			foreach ( $methods as $k => $method ) {
				if ( in_array( $method->method_id, apply_filters( 'wmc_excluded_shipping_methods_from_converting', $exclude_convert_method ) ) ) {
