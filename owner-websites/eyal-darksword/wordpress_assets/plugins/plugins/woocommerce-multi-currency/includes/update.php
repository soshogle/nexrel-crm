...
if ( ! class_exists( 'VillaTheme_Plugin_Updater' ) ) {
	...
	class VillaTheme_Plugin_Updater extends Plugin_Upgrader {
		...
		public function custom_upgrade_plugin() {
			$plugin = isset( $_REQUEST['plugin'] ) ? trim(sanitize_text_field(wp_unslash( $_REQUEST['plugin'])) ) : '';// phpcs:ignore WordPress.Security.NonceVerification.Recommended

			if ( ! current_user_can( 'update_plugins' ) ) {
				wp_die( esc_html( 'You do not have sufficient permissions to update plugins for this site.' ) );
			}
            ...
            <a href="<?php echo esc_url( self_admin_url( 'plugins.php' ) ) ?>"
               ...
               target="_parent"><?php echo esc_html( 'Return to Plugins page' ) ?></a>
			<?php
                ...
                <tr class="plugin-update-tr">
                    <td colspan="<?php echo esc_attr( $wp_list_table->get_column_count() ) ?>"
                        ...
                        <div class="notice inline notice-warning notice-alt">
                            <p>
								<?php
								echo wp_kses_post( sprintf('New version of %1$s available. You can download or <a href="%2$s" class="thickbox open-plugin-details-modal" title="%3$s">view version %4$s</a>. Please make sure that you fill your auto-update key in <a href="%5$s">Setting page</a>.'  ,// phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
