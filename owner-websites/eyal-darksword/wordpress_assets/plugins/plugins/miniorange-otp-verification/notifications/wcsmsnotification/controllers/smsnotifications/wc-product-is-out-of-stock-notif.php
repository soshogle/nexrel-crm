...
$sms_settings = $notification_settings->wc_product_is_out_of_stock_notif;
...
// Secure deserialization of recipient data with validation.
$recipient_value = '';
if ( isset( $sms_settings->recipient ) && ! empty( $sms_settings->recipient ) ) {
	if ( is_serialized( $sms_settings->recipient ) ) {
		$recipient_data = maybe_unserialize( $sms_settings->recipient );
		// Validate deserialized data is an array before processing.
		if ( is_array( $recipient_data ) ) {
			// Sanitize each recipient value as phone numbers.
			$sanitized_recipients = array_map(
				static function ( $r ) {
			...
			$recipient_value      = implode( ';', $sanitized_recipients );
		} else {
			$recipient_value = MoUtility::process_phone_number( (string) $recipient_data );
		}
	} else {
