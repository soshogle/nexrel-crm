...
if ( is_string( $notification_settings ) && is_serialized( $notification_settings ) ) {
	$tmp                   = maybe_unserialize( $notification_settings );
	$notification_settings = ( is_object( $tmp ) && method_exists( $tmp, 'get_wc_order_pending_notif' ) )
		? $tmp
		: WooCommerceNotificationsList::mo_otp_get_instance();
} elseif ( ! ( is_object( $notification_settings ) && method_exists( $notification_settings, 'get_wc_order_pending_notif' ) ) ) {
...
$sms_settings = $notification_settings->wc_order_pending_notif;
...
if ( defined( 'MSN_DIR' ) && ! empty( MSN_DIR ) ) {
	$template_file = MSN_DIR . '/views/smsnotifications/wc-customer-sms-template.php';
