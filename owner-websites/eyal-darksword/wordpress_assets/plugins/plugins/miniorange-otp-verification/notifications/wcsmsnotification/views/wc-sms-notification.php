...
// Access control check - only administrators can view SMS notification settings.
...
$wc_hidden = isset( $wc_hidden ) ? sanitize_html_class( $wc_hidden ) : '';
$nonce     = isset( $nonce ) ? sanitize_key( $nonce ) : '';
$disabled  = isset( $disabled ) ? sanitize_text_field( $disabled ) : '';

// Validate notification settings object.
if ( ! $notification_settings || ! is_object( $notification_settings ) ) {
	wp_die( esc_html( MoMessages::showMessage( MoMessages::UNKNOWN_ERROR ) ) );
}

// Validate nonce for form security.
if ( empty( $nonce ) ) {
	wp_die( esc_html( MoMessages::showMessage( MoMessages::UNKNOWN_ERROR ) ) );
}
?>

<div id="MowcNotifSubTabContainer" class="mo-subpage-container <?php echo esc_attr( $wc_hidden ); ?>">
	<form name="f" 
		method="post" 
		action="" 
		id="mo_wc_sms_notif_settings">

		<!-- Hidden Fields -->
		<input type="hidden" 
			name="option" 
			value="mo_wc_sms_notif_settings" />
		<?php wp_nonce_field( $nonce ); ?>

		<!-- Header Section -->
		<div class="mo-header">
			<p class="mo-heading flex-1">
				<?php echo esc_html( __( 'WooCommerce SMS Notification Settings', 'miniorange-otp-verification' ) ); ?>
			</p>

			<!-- Setup Guide Link -->
			<u>
				<i>
					<a href="<?php echo esc_url( 'https://plugins.miniorange.com/how-to-configure-woocommerece-sms-notification' ); ?>" 
						target="_blank"
			...
			<input type="submit" 
				...
				value="<?php echo esc_attr( __( 'Save Settings', 'miniorange-otp-verification' ) ); ?>">
		</div>
		...
		<table class="mo-wcnotif-table bg-white">
			<thead>
				<tr>
					<th scope="col"><?php echo esc_html( __( 'SMS Type', 'miniorange-otp-verification' ) ); ?></th>
					<th scope="col"><?php echo esc_html( __( 'Recipient', 'miniorange-otp-verification' ) ); ?></th>
					<th scope="col"></th>
					<th scope="col"><?php echo esc_html( __( 'SMS Body', 'miniorange-otp-verification' ) ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php
					OTP\Notifications\WcSMSNotification\Controllers\mo_show_wc_notifications_table( $notification_settings );
				?>
			</tbody>
		</table>
	</form>
