...
if ( is_string( $notification_settings ) && is_serialized( $notification_settings ) ) {
	...
	$notification_settings = ( is_object( $tmp ) && method_exists( $tmp, 'get_wc_order_refunded_notif' ) )
		? $tmp
		: WooCommerceNotificationsList::mo_otp_get_instance();
} elseif ( ! is_object( $notification_settings ) ) {
...
$sms_settings = $notification_settings->wc_order_refunded_notif;
