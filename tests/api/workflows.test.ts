...
describe("Workflows API", () => {
  ...
  describe("GET /api/workflows", () => {
    it("should return workflows for user industry", async () => {
      ...
      if (!response) {
        throw new Error(
          "GET returned undefined - route handler may have thrown an error",
        );
      }
  ...
  describe("GET /api/workflows/templates", () => {
    it("should return industry templates", async () => {
      const request = new Request(
        "http://localhost:3000/api/workflows/templates",
      );
    ...
    it("should return specific template by ID", async () => {
      const request = new Request(
        "http://localhost:3000/api/workflows/templates?id=patient-onboarding",
      );
      const response = await GETTemplate(request as any);
      const data = await response.json();
  ...
  describe("POST /api/workflows/[id]/execute", () => {
    it("should execute workflow instance", async () => {
      ...
      const request = new Request(
        "http://localhost:3000/api/workflows/workflow-1/execute",
        {
          method: "POST",
          body: JSON.stringify(requestBody),
        },
    ...
    it("should fail if workflow not found", async () => {
      ...
      const requestBody = { leadId: "lead-1" };
      const request = new Request(
        "http://localhost:3000/api/workflows/invalid/execute",
        {
          method: "POST",
          body: JSON.stringify(requestBody),
        },
  ...
  describe("POST /api/workflows/hitl/[id]/approve", () => {
    it("should approve HITL gate", async () => {
      ...
      const { approveHITLGate } = await import(
      ...
      const request = new Request(
        "http://localhost:3000/api/workflows/hitl/execution-1/approve",
        {
          method: "POST",
          body: JSON.stringify(requestBody),
        },
      ...
      let response;
      try {
        response = await POSTApprove(request as any, {
          params: { id: "execution-1" },
        });
      } catch (error) {
  ...
  describe("POST /api/workflows/hitl/[id]/reject", () => {
    it("should reject HITL gate", async () => {
      ...
      const { rejectHITLGate } = await import(
        "@/lib/workflows/workflow-engine"
      );
      ...
      const request = new Request(
        "http://localhost:3000/api/workflows/hitl/execution-1/reject",
        {
          method: "POST",
          body: JSON.stringify(requestBody),
        },
      ...
      let response;
      try {
        response = await POSTReject(request as any, {
          params: { id: "execution-1" },
        });
      } catch (error) {
