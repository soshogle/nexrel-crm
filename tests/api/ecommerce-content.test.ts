...
const TEST_WEBSITE_ID =
  process.env.TEST_WEBSITE_ID || "cmlkjpjlq0001jr043j95fl94";
...
describe("ecommerce-content API", () => {
  beforeAll(async () => {
    ...
    if (!site) {
      console.warn(
        `Test website ${TEST_WEBSITE_ID} not found - some tests may be skipped`,
      );
    }
  });
  ...
  it("returns ecommerce content structure", async () => {
    ...
    const res = await fetch(
      `${base}/api/websites/${TEST_WEBSITE_ID}/ecommerce-content`,
      {
        headers,
      },
    );
