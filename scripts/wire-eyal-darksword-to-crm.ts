...
async function main() {
  ...
  if (!vercelProjectId)
    vercelProjectId = await findVercelProject("darksword-armory-website");
  if (!vercelProjectId)
    vercelProjectId = await findVercelProject("darksword-armory", false); // personal account
  if (!vercelProjectId)
    vercelProjectId = await findVercelProject(
  ...
  if (!vercelProjectId) {
    ...
    console.warn(
      '   Set VERCEL_TOKEN in .env. If projects are under a personal account, try VERCEL_TEAM_ID=""',
    );
    console.warn(
      "   You can still set githubRepoUrl; vercelProjectId will be null.",
    );
  } else {
