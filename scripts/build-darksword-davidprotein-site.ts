...
function buildFullStructure(darksword: any): any {
  ...
  const heroImg =
    images.find((u: string) => u.includes("banner") || u.includes("home")) ||
    images[0];
  ...
  const aboutImg =
    images.find((u: string) => u.includes("elite") || u.includes("banner")) ||
    images[5];

  const products = (darksword.products || [])
    .slice(0, 6)
    .map((p: any, i: number) => ({
      name: p.name,
  ...
  const aboutParagraph =
    (darksword.paragraphs || [])[0]?.replace(/&nbsp;/g, " ") ||
        ...
        components: [
              ...
              services: [
                {
                  name: "Elite Series",
                  description:
                    "Damascus steel swords. Premium craftsmanship for collectors and enthusiasts.",
                  image: tierImg1,
                },
                {
                  name: "Herald Series",
                  description:
                    "Battle-ready medieval weapons. Faithful to historical weight, balance and feel.",
                  image: tierImg2,
                },
              ],
              ctaText: "Shop All",
          ...
          // 6. Testimonials placeholder (David Protein: "The David community")
          {
            ...
            props: {
              ...
              description:
                "Collectors, HEMA practitioners, and enthusiasts trust Darksword Armory for battle-ready steel.",
...
async function main() {
  ...
  const outPath = path.join(
    process.cwd(),
    "darksword-davidprotein-structure.json",
  );
  ...
  if (shouldCreate) {
    ...
    if (!targetUserId) {
      console.error(
        "No target userId. Pass --userId=YOUR_USER_ID or create eyal@darksword-armory.com first.",
      );
      process.exit(1);
    }
    ...
    console.log(
      `  Full URL: ${process.env.NEXTAUTH_URL || "http://localhost:3000"}/dashboard/websites/${websiteId}`,
    );
  } else {
