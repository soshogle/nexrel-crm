...
function pdColor(pd: number) {
  return pd <= 3 ? "#10b981" : pd <= 6 ? "#f59e0b" : "#ef4444";
}
function pdGlowRgba(pd: number) {
  return pd <= 3
  ...
  return (
    ...
    >
      ...
      {t === "molar" && (
        <>
          ...
          <line
            ...
            y1={grooveY1}
            x2={w * 0.36}
            y2={isUpper ? grooveY1 + h * 0.15 : grooveY1 - h * 0.15}
            stroke="rgba(140,112,60,0.14)"
            strokeWidth="0.6"
            strokeLinecap="round"
          />
          <line
            ...
            y1={grooveY1}
            x2={w * 0.8}
            y2={isUpper ? grooveY1 + h * 0.12 : grooveY1 - h * 0.12}
            stroke="rgba(140,112,60,0.14)"
            strokeWidth="0.6"
            strokeLinecap="round"
          />
      ...
      {t === "canine" && (
        <line
          x1={w * 0.5}
          y1={isUpper ? h * 0.05 : h * 0.95}
          x2={w * 0.5}
          y2={isUpper ? h * 0.6 : h * 0.4}
      ...
      {t === "incisor" && (
        <>
          <line
            x1={w * 0.12}
            y1={isUpper ? h * 0.06 : h * 0.94}
            x2={w * 0.88}
            y2={isUpper ? h * 0.06 : h * 0.94}
      ...
      <text
        x={w / 2}
        y={isUpper ? h * 0.62 : h * 0.42}
        textAnchor="middle"
        dominantBaseline="middle"
        fontSize="5.5"
        fill="rgba(110,85,30,0.55)"
...
const SITE_FULL_NAMES: Record<string, string> = {
  M: "Mesial",
  B: "Buccal",
  D: "Distal",
  L: "Lingual",
    ...
    >
      ...
      >
        ...
        <span
          style={{
            position: "absolute",
            [isUpper ? "bottom" : "top"]: zoomed
              ? "calc(100% + 6px)"
              : "calc(100% + 3px)",
            left: "50%",
            transform: "translateX(-50%)",
            fontSize: zoomed ? 14 : 6,
            color: col,
      ...
      <span
        style={{
          fontSize: zoomed ? 10 : 4.5,
          color: isL ? "rgba(148,163,184,0.35)" : "rgba(148,163,184,0.5)",
          fontFamily: "monospace",
          marginTop: isUpper ? (zoomed ? 4 : 1) : 0,
          marginBottom: isUpper ? 0 : zoomed ? 4 : 1,
        ...
        <>
          ...
          <div
            style={{
              ...
              margin: zoomed ? "6px 0 4px" : "3px 0 2px",
              borderRadius: 2,
            }}
          />
          <div
        ...
        <>
          ...
          <div
            style={{
              ...
              background: col,
              boxShadow: `0 0 8px ${glow}, 0 0 2px ${col}`,
              margin: zoomed ? "4px 0 6px" : "2px 0 3px",
              borderRadius: 2,
            }}
          />
          <CrownSvg
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                ...
                >
                  ...
                  <input
                    ...
                    value={pd || ""}
                    onChange={(e) =>
                      onChange(key, "pd", parseInt(e.target.value) || 0)
                    }
                    placeholder="0"
                    style={{
                      width: 44,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Tooth {num} · {toothType(num)} ·{" "}
          {isUpper ? "Maxillary" : "Mandibular"}
        </span>
      </div>
...
export function NeuralPerioChart({
  measurements,
  leadId,
  onSave,
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  // ── Stats ──
  const allPds = Object.values(localData)
    .flatMap((d) => ALL_SITES.map((s) => d[s.key]?.pd ?? 0))
    .filter(Boolean);
  const bopCount = Object.values(localData).reduce(
    (s, d) => s + ALL_SITES.filter((x) => d[x.key]?.bop).length,
    0,
  );
  const deepCount = allPds.filter((p) => p > 6).length;
  const healthPct = allPds.length
    ? Math.round((allPds.filter((p) => p <= 3).length / allPds.length) * 100)
    : 0;
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(170deg,#060b1a 0%,#0a1030 45%,#080d22 100%)",
        borderRadius: 14,
      ...
      >
        ...
        <div
          style={{
            ...
            height: 1,
            background:
              "linear-gradient(90deg,rgba(129,140,248,0.4),transparent)",
            minWidth: 20,
          }}
        />
        ...
        <button
          ...
          style={{
            ...btnBase,
            background:
              viewMode === "2d" ? "rgba(99,102,241,0.35)" : btnBase.background,
            borderColor:
              viewMode === "2d"
        ...
        <button
          ...
          style={{
            ...btnBase,
            background:
              viewMode === "3d" ? "rgba(99,102,241,0.35)" : btnBase.background,
            borderColor:
              viewMode === "3d"
                ? "rgba(129,140,248,0.6)"
                : "rgba(129,140,248,0.3)",
          ...
          {examHistory.length === 0 ? (
            ...
            >
              {leadId
                ...
                : "Select a patient to load history"}
            </span>
          ) : (
            <>
              <select
        ...
        <>
          ...
          {!selectedTooth && (
            ...
            >
              ...
              >
                click any tooth to zoom in · M B D L = mesial buccal distal
                lingual
              </span>
            </div>
          )}
              ...
              >
                <div
                  style={{
                    ...
                    background: s.col,
                    borderRadius: 2,
                    boxShadow: `0 0 6px ${s.col}80`,
                  }}
                />
                <span
                  style={{
                    fontSize: 8,
                    color: "rgba(188,200,230,0.75)",
                    fontFamily: "monospace",
                  }}
                >
                  {s.label}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <div
                style={{
                  ...
                  height: 9,
                  background:
                    "radial-gradient(ellipse at 50% 40%, #ff4444 40%, #cc0000 100%)",
                  clipPath: "polygon(50% 100%, 0% 0%, 100% 0%)",
