...
export function EnhancedPeriodontalChart({
  measurements,
  patient: _patient,
  ...
  const PdDataGrid = ({
    ...
    sites: { site: string; d: SiteData | undefined }[];
  }) => (
      ...
      {sites.map(({ site, d }) => {
        ...
        return (
          <div key={site} className="flex items-center gap-px">
            <span
              className="text-[5px] font-medium text-white/50 shrink-0"
              style={{ width: 6 }}
            >
  ...
  const PerioTooth = ({
    ...
    const sites = SITES.map((s) => ({
      site: s,
      d: data[s as keyof ToothMeasurements],
    }));
    ...
    return (
      ...
      >
        {isUpper ? (
          <>
            <span className="text-[7px] font-medium text-white/80 leading-none mb-0.5">
              {toothNum}
            </span>
            <ToothSvg />
            <div className="mt-0.5 w-full px-px">
              <PdDataGrid sites={sites} />
            </div>
          </>
        ) : (
          <>
            <div className="mb-0.5 w-full px-px">
              <PdDataGrid sites={sites} />
            </div>
            <ToothSvg />
            <span className="text-[7px] font-medium text-white/80 leading-none mt-0.5">
              {toothNum}
            </span>
          </>
        )}
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center justify-between py-2 mt-2 rounded-lg bg-slate-900/90 text-white text-[10px] px-3">
        <button
          type="button"
          className="p-1.5 rounded hover:bg-white/10 text-white/90 transition-colors"
        >
