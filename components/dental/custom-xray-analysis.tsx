...
function deriveFindings(xrayData?: any, toothData?: Record<string, any>): {
  restorations: string[];
  boneLevel: string;
  periapical: string;
  boneDensity: string;
  ...
  if (src && typeof src === 'object') {
    for (const [num, info] of Object.entries(src)) {
      ...
      if (cond === 'filling') {
        defaults.restorations.push(`Tooth #${num}: Existing composite restoration — intact, no secondary caries`);
      } else if (cond === 'crown') {
        defaults.restorations.push(`Tooth #${num}: Crown — margins intact`);
      } else if (cond === 'caries') {
        defaults.restorations.push(`Tooth #${num}: Caries detected — ${treat || 'treatment recommended'}`);
      } else if (cond === 'root_canal') {
        defaults.restorations.push(`Tooth #${num}: Root canal therapy — periapical status WNL`);
      } else if (cond === 'implant') {
        defaults.restorations.push(`Tooth #${num}: Implant — osseointegration stable`);
      } else if (cond === 'missing' || cond === 'extraction') {
        defaults.restorations.push(`Tooth #${num}: ${cond === 'missing' ? 'Missing' : 'Extracted'}`);
      }
    }
  }
...
export function CustomXRayAnalysis({ xrayData, toothData }: XRayAnalysisProps) {
  const findings = useMemo(() => deriveFindings(xrayData, toothData), [xrayData, toothData]);
    ...
    <div className="relative">
      ...
      <div className="relative w-full h-48 bg-black rounded-lg flex items-center justify-center mb-3 overflow-hidden">
        ...
        {imageUrl && !imageError ? (
          <>
            ...
            <img
              src={imageUrl}
              alt={xrayData?.xrayType ? `${xrayData.xrayType} X-Ray` : 'Dental X-Ray'}
          ...
          <>
            ...
            <div className="relative flex flex-col items-center justify-center text-center z-10">
              ...
              <p className="text-[11px] text-gray-500 font-medium">
                {imageError ? 'Failed to load X-ray image' : 'No X-ray image uploaded'}
              </p>
        ...
        {(imageLoaded || (!imageUrl && hasFindings)) && (
          <>
            <div className="absolute top-1.5 right-1.5 bg-white/90 border border-purple-300 rounded px-1.5 py-0.5 shadow-sm z-10">
              <div className="text-[9px] font-medium text-gray-900">Bone Level</div>
              <div className={`text-[9px] font-semibold ${findings.boneLevel.includes('Normal') || findings.boneLevel.includes('Healthy') ? 'text-green-600' : 'text-amber-600'}`}>
                {findings.boneLevel}
              </div>
            </div>

            <div className="absolute bottom-1.5 left-1.5 bg-white/90 border border-purple-300 rounded px-1.5 py-0.5 shadow-sm z-10">
              <div className="text-[9px] font-medium text-gray-900">Periapical</div>
              <div className={`text-[9px] ${findings.periapical.includes('No pathology') ? 'text-green-600' : 'text-amber-600'}`}>
                {findings.periapical}
              </div>
            </div>

            <div className="absolute bottom-1.5 right-1.5 bg-white/90 border border-purple-300 rounded px-1.5 py-0.5 shadow-sm z-10">
              <div className="text-[9px] font-medium text-gray-900">Density</div>
              <div className={`text-[9px] font-semibold ${findings.boneDensity.includes('Healthy') ? 'text-green-600' : 'text-amber-600'}`}>
      ...
      {xrayData?.aiAnalysis && (
        <div className="mt-2 text-[9px] text-gray-400 italic">
          AI analysis for informational purposes only — not for diagnostic use. Professional review required.
        </div>
      )}
    </div>
  );
