...
function sevColor(sev: string) {
  return SEV_COLORS[sev] || SEV_COLORS.healthy;
}
...
export function PerioAiSidePanel({
  ...
  const s: React.CSSProperties = {
    ...
    background:
      "linear-gradient(175deg, #070c1e 0%, #0b1132 50%, #080d24 100%)",
    ...
    return (
      <div style={s}>
        ...
        >
          ...
          >
            Run AI analysis to get a severity assessment, per-tooth findings,
            and treatment suggestions — combining periodontal probing data
            {leadId ? " and any existing x-ray analyses" : ""}.
          </p>
  ...
  return (
    <div style={s}>
      ...
      >
        ...
        <span
          style={{ fontSize: 10, color: "#cbd5e1", fontFamily: "monospace" }}
        >
      ...
      {metrics && (
        ...
        >
          {[
            ...
            {
              label: `${metrics.worstPd}mm worst`,
              col:
                metrics.worstPd > 6
                  ? "#ef4444"
                  : metrics.worstPd > 3
        ...
        >
          {analysis.keyFindings.map((f, i) => (
            <div key={i} style={{ display: "flex", gap: 6, marginBottom: 6 }}>
              <span style={{ color: "#818cf8", fontSize: 8, marginTop: 1 }}>
                ●
              </span>
              <span
                style={{
      ...
      {analysis.toothFindings.length > 0 && (
        <Section
          ...
          count={analysis.toothFindings.length}
          expanded={expandedSections.teeth}
          onToggle={() => toggle("teeth")}
        >
      ...
      {analysis.suggestedActions.length > 0 && (
        ...
        >
          {analysis.suggestedActions.map((a, i) => (
            <div key={i} style={{ display: "flex", gap: 6, marginBottom: 6 }}>
              <span style={{ color: "#10b981", fontSize: 8, marginTop: 1 }}>
                ▸
              </span>
              <span
                style={{
      ...
      >
        ...
        >
          {analysis.disclaimer ||
            "AI-assisted clinical decision support. Not a diagnosis. All findings require verification by a licensed dental professional. Not a substitute for professional judgment."}
        </p>
      </div>
    </div>
  );
