...
export function ClinicalModals({
  ...
  return (
    <>
                ...
                <Button
                  ...
                  onClick={() => {
                    if (selectedLeadId && safeLeads.length > 0) {
                      const currentIndex = safeLeads.findIndex(
                  ...
                  disabled={
                    !selectedLeadId ||
                    (safeLeads.length > 0 &&
                      safeLeads.findIndex((l) => l.id === selectedLeadId) === 0)
                  }
                >
                  <ChevronLeft
                    className={`h-4 w-4 ${!selectedLeadId || (safeLeads.length > 0 && safeLeads.findIndex((l) => l.id === selectedLeadId) === 0) ? "text-gray-300" : "text-gray-600"}`}
                  />
                </Button>
                <Button
                  variant="ghost"
                  ...
                  disabled={
                    !selectedLeadId ||
                    (safeLeads.length > 0 &&
                      safeLeads.findIndex((l) => l.id === selectedLeadId) ===
                        safeLeads.length - 1)
                  }
                >
                  <ChevronRight
                    className={`h-4 w-4 ${!selectedLeadId || (safeLeads.length > 0 && safeLeads.findIndex((l) => l.id === selectedLeadId) === safeLeads.length - 1) ? "text-gray-300" : "text-gray-600"}`}
                  />
                </Button>
              </div>
            </div>
        ...
        {!selectedLeadId ? (
          ...
          <div className="h-[calc(100vh-200px)]">
            {safeXrays.length > 1 && (
              <div className="mb-4 flex items-center gap-2">
                <Select
                  ...
                  onValueChange={(value) => {
                    const xray = safeXrays.find((x: any) => x.id === value);
                    if (xray) onSelectXray(xray);
                  }}
                >
            ...
            <DicomViewer
              ...
              imageUrl={
                selectedXray?.fullUrl ||
                selectedXray?.previewUrl ||
                selectedXray?.imageUrl ||
                `/api/dental/xrays/${selectedXray?.id}/image`
              }
          ...
          procedures={safeProcedures.map((proc: any) => {
            ...
            return {
              time: procDate
                ...
                : "--",
              patient:
                safeLeads.find((l) => l.id === proc.leadId)?.contactPerson ||
                "Unknown",
              procedure:
                proc.procedureCode || proc.procedureName || "Procedure",
      ...
      <CardModal
        isOpen={openModal === "photo-gallery"}
        onClose={onCloseModal}
      ...
      <CardModal
        isOpen={openModal === "stl-viewer"}
        onClose={onCloseModal}
        title="3D Intraoral Scans"
      >
        {selectedLeadId ? (
            ...
            <div className="flex items-center justify-between mb-2">
              <Button
                ...
                onClick={() => {
                  ...
                  const patientName =
                    safeLeads.find((l) => l.id === selectedLeadId)
                      ?.contactPerson ||
                    safeLeads.find((l) => l.id === selectedLeadId)
                      ?.businessName;
      ...
      <CardModal
        isOpen={openModal === "predictive-outcome"}
        onClose={onCloseModal}
        title="Predictive Outcome Simulator"
      >
        {selectedLeadId ? (
      ...
      <CardModal
        isOpen={openModal === "xray-multi-view"}
        onClose={onCloseModal}
        title="Multi-Angle X-Ray Viewer"
      >
        {selectedLeadId ? (
          <XRayMultiView
