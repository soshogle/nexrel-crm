...
export default function CallHistoryPanel({ selectedConversationId, source = 'elevenlabs' }: CallHistoryPanelProps) {
  ...
  // Debug logging
  useEffect(() => {
    console.log('🔍 [CallHistoryPanel] Session debug:', {
      hasSession: !!session,
  ...
  const getTranscriptTurnCount = (conversation: any) => {
    // Use message_count from list API, or transcript.length from detail API
    if (conversation?.message_count) return conversation.message_count;
    if (conversation?.transcript && Array.isArray(conversation.transcript)) return conversation.transcript.length;
    return 0;
  };
  ...
  const handleCopyTranscript = () => {
    ...
    const transcriptText = conversationDetails.transcript
      .map((turn: any) => {
        ...
        const speaker = turn.role === 'agent' ? (conversationDetails.agent_name || 'AI Agent') : 'Caller';
  ...
  const handleDownloadTranscript = () => {
    ...
    // Add header
    const header = `Call Transcript\nDate: ${format(new Date(selectedConversation.start_time_unix_secs * 1000), "MMM d, yyyy 'at' h:mm a")}\nDuration: ${formatDuration(selectedConversation.call_duration_secs)}\nAgent: ${conversationDetails.agent_name || 'Unknown'}\n\n---\n\n`;
    ...
    link.href = url;
    link.download = `transcript_${selectedConversation.conversation_id}_${format(new Date(selectedConversation.start_time_unix_secs * 1000), 'yyyy-MM-dd')}.txt`;
  ...
  const uniqueAgents = Array.from(
    new Set(conversations.map((c) => c.agent_id))
  ).map((agent_id) => {
    const conv = conversations.find((c) => c.agent_id === agent_id);
    return { id: agent_id, name: conv?.agent_name || agent_id };
  });
    ...
    return (
      <div className="flex items-center justify-center h-[600px] bg-white">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm text-gray-600">Loading call history...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {/* Debug: Show role and impersonation status for testing */}
                {session?.user?.email && (
                  <>
              ...
              <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-center h-32" ref={waveformRef}>
                    ...
                    <div className="flex items-center justify-center gap-[2px] h-full w-full">
                      {Array.from({ length: 120 }).map((_, i) => {
                        ...
                        // Create more realistic waveform pattern with varying heights
                        const baseHeight = 30 + Math.sin(i / 10) * 20 + Math.random() * 40;
                ...
                {conversationDetails?.audio_url ? (
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
                    <p className="text-gray-600">No audio recording available for this conversation</p>
                  </div>
                )}
                ...
                <Tabs defaultValue="transcription" className="w-full">
                  <div className="flex items-center justify-between border-b border-gray-200">
                    ...
                    <div className="flex items-center gap-2 mr-2">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={handleCopyTranscript} disabled={!conversationDetails?.transcript}>
                            <Copy className="h-4 w-4 mr-2" />
                            Copy transcript
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={handleDownloadTranscript} disabled={!conversationDetails?.transcript}>
                            <Download className="h-4 w-4 mr-2" />
                            Download transcript
                          </DropdownMenuItem>
                          {/* Debug: Show role and impersonation status for testing */}
                  ...
                  <TabsContent value="transcription" className="mt-4">
                    {conversationDetails?.transcript && Array.isArray(conversationDetails.transcript) && conversationDetails.transcript.length > 0 ? (
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-sm font-medium text-gray-900">
                                  {turn.role === 'agent' ? (conversationDetails.agent_name || 'AI Agent') : 'Caller'}
                                </span>
                                <span className="text-xs text-gray-500">
                                  {formatTime(turn.time_in_call_secs || 0)}
                                </span>
                              </div>
                              <p className="text-sm text-gray-700 leading-relaxed">{turn.message}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="text-center py-8">
                        <p className="text-sm text-gray-500">No transcript available for this conversation</p>
                        {conversationDetails && !conversationDetails.transcript && (
                          <p className="text-xs text-gray-400 mt-2">
                            This call may have been too short or ended before a full conversation occurred.
                          </p>
                        )}
                      </div>
                    )}
                  ...
                  <TabsContent value="client-data" className="mt-4">
                    <div className="space-y-3">
                      <div className="flex justify-between py-2">
                        ...
                        <span className="text-sm font-medium text-gray-900">
                          {selectedConversation.conversation_initiation_client_data?.caller_number || 'Unknown'}
                        </span>
                      </div>
                      <div className="flex justify-between py-2">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-900">
                  {format(new Date(selectedConversation.start_time_unix_secs * 1000), "MMM d, yyyy 'at' h:mm a")}
                </p>
              </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-center p-8">
            ...
            <p className="text-sm text-gray-500 mt-1">
              {searchQuery || selectedAgent !== 'all' || selectedStatus !== 'all'
                ...
                : 'Call history will appear here once you make calls'}
            </p>
          </div>
        ) : (
              ...
              {filteredConversations.map((conversation) => (
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {format(new Date(conversation.start_time_unix_secs * 1000), 'MMM d, h:mm a')}
                  </td>
