...
export default function CalendarPage() {
  ...
  const stats = useMemo(() => {
    ...
    for (const apt of appointments) {
      try {
        if (!apt?.startTime || !apt?.status || apt.status === "CANCELLED")
          continue;
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-purple-300 mt-0.5 text-sm">
              Manage your {config.bookingPluralNoun.toLowerCase()} and schedule
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="glass-effect border-purple-500/20 hover:border-purple-500/30 transition-colors">
          ...
          <CardContent>
            ...
            <p className="text-xs font-bold text-purple-400 mt-1">
              {config.bookingPluralNoun.toLowerCase()} today
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="glass-effect border-purple-500/20 hover:border-purple-500/30 transition-colors">
          ...
          <CardContent>
            ...
            <p className="text-xs font-bold text-purple-400 mt-1">
              all {config.bookingPluralNoun.toLowerCase()}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="calendar" className="space-y-4">
        <TabsList className="bg-black/40 border border-purple-500/20">
          <TabsTrigger
            value="calendar"
            className="data-[state=active]:gradient-primary data-[state=active]:text-white text-white/80 data-[state=active]:shadow-lg data-[state=active]:shadow-purple-500/30"
          >
