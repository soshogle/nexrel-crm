...
export default function DocpenPage() {
  ...
  const [externalDialogActive, setExternalDialogActive] =
    useState(isExternalOpen);

  const [activeView, setActiveView] = useState<"list" | "session">(
    sessionIdParam ? "session" : "list",
  );
  ...
  useEffect(() => {
    ...
    if (userIndustry && !hasDocpenAccess) {
      return;
    }
  ...
  }, [sessionIdParam, sessionStatus, session, hasDocpenAccess, userIndustry]);
  ...
  const fetchStats = async () => {
    try {
      ...
      const soapNotesGenerated = sessions.filter(
        (s: Session) => s.soapNoteGenerated,
      ).length;

      const signedComplete = sessions.filter(
        (s: Session) => s.status === "SIGNED" || s.signedAt,
      ).length;
  ...
  const currentSOAPNote =
    activeSession?.soapNotes?.find((n) => n.isCurrentVersion) || null;
      ...
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          <p className="text-muted-foreground">
            Please sign in to access Docpen
          </p>
        </div>
      </div>
    );
      ...
      <div className="container mx-auto py-6 space-y-6">
            ...
            {activeView === "list" && (
              <>
                ...
                <NewSessionDialog
                  ...
                  defaultChiefComplaint={chiefComplaintParam || undefined}
                  defaultProfession={
                    fromParam === "clinical" ? "DENTIST" : undefined
                  }
                  defaultOpen={false}
                  industry={userIndustry}
                />
              ...
              <CardContent className="pt-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold">
                      {sessionStats.soapNotesGenerated}
                    </p>
                  </div>
                </div>
              </CardContent>
          ...
          <div className="space-y-6">
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                {Array.isArray(activeSession?.transcriptions) &&
                  activeSession.transcriptions.length > 0 && (
                    <Card>
                      <CardHeader>
                        <CardTitle className="text-lg flex items-center gap-2">
                          ...
                          <Badge variant="outline">
                            {activeSession.transcriptions.length} segments
                          </Badge>
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                  ...
                  <Tabs defaultValue="voice" className="w-full">
                    ...
                    <TabsContent value="voice" className="mt-0">
                      <VoiceAssistant
                        sessionId={activeSession.id}
                        profession={
                          activeSession.profession as DocpenProfessionType
                        }
        ...
        {activeSession && (
          <ReviewSignDialog
            ...
            sessionId={activeSession.id}
            patientName={
              activeSession.patientName || activeSession.lead?.contactPerson
            }
