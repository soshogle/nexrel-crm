...
export default function AdministrativeDashboardPage() {
  ...
  const fetchClaims = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const nextClaims = Array.isArray(data)
          ? data
        ...
        const outstanding = nextClaims
          ...
          .reduce(
            (sum: number, c: any) =>
              sum + Number(c.amount || c.estimatedCost || 0),
            0,
          );
  ...
  const fetchLabOrders = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/dental/lab-orders${params ? `?${params}` : ""}`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchFormResponses = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/dental/forms?type=responses${leadParam}`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchProductionMetrics = useCallback(async () => {
    try {
      ...
      const week = weekRes.ok
        ? await weekRes.json()
        : { summary: {}, data: [] };
      const month = monthRes.ok
        ? await monthRes.json()
        : { summary: {}, data: [] };
      ...
      const prevDayRevenue =
        dayData.length >= 2
          ? Number(dayData[dayData.length - 2]?.revenue || 0)
          : 0;
      const revenueTrend =
        prevDayRevenue > 0
          ? ((dayRevenue - prevDayRevenue) / prevDayRevenue) * 100
          : 0;

      const providerAgg = new Map<
        string,
        { cases: number; production: number }
      >();
      ...
      const apptCount = Math.max(
        1,
        Array.isArray(appointments) ? appointments.length : 0,
      );
      for (const [provider, value] of providerAgg.entries()) {
        value.production = Math.round(
          (value.cases / apptCount) * Number(monthSummary.totalRevenue || 0),
        );
        providerAgg.set(provider, value);
      }
      const computedTeam = Array.from(providerAgg.entries()).map(
        ([name, v], idx) => ({
          id: `provider-${idx}`,
          name,
          role: "Provider",
          ...
          efficiency: Math.min(
            100,
            Math.max(50, Math.round((v.cases / apptCount) * 100)),
          ),
      ...
      const chairUtilization = Math.min(
        100,
        Math.round(
          ((Array.isArray(appointments) ? appointments.length : 0) / 16) * 100,
        ),
      );
      const teamProductivity =
        computedTeam.length > 0
          ? Math.round(
              computedTeam.reduce((sum, t) => sum + t.efficiency, 0) /
                computedTeam.length,
            )
          : 0;
      ...
      setProductionMetrics({
        ...
        teamProductivity,
        productionTrend:
          revenueTrend > 1 ? "up" : revenueTrend < -1 ? "down" : "stable",
        revenueTrend,
      });
      ...
      const byDayOfWeek = Array.from(byDayMap.entries()).map(
        ([day, value]) => ({ day, value }),
      );
      ...
      setProductionChartData({
        ...
        monthlyData,
        byTreatmentType: [],
        byPractitioner: computedTeam.map((m) => ({
          name: m.name,
          value: m.production,
        })),
        byDayOfWeek,
  ...
  const displayMultiChairAppointments = appointments.map((apt: any) => ({
    chair: apt.chair || "1",
    time: new Date(apt.startTime || apt.appointmentDate).toLocaleTimeString(
      "en-US",
      { hour: "2-digit", minute: "2-digit" },
    ),
    patient:
      leads.find((l) => l.id === apt.leadId)?.contactPerson ||
      apt.customerName ||
      "Unknown",
  ...
  const handleCheckInSelectedPatient = useCallback(async () => {
    if (!selectedLeadId) {
  ...
  const displayClaims = claims.slice(0, 5).map((claim: any) => ({
    ...
    status:
      String(claim.status || "").toUpperCase() === "APPROVED"
        ? "Approved"
        : ["SUBMITTED", "UNDER_REVIEW", "PENDING", "INFO_REQUESTED"].includes(
  ...
  const displayFormResponses = formResponses
    .filter((response: any) => {
      ...
      const patientName =
        leads.find((l) => l.id === response.leadId)?.contactPerson || "Unknown";
  ...
  const isAdmin =
    session?.user?.role === "ADMIN" || session?.user?.email?.includes("admin");
              ...
              <RedesignedCheckIn
                patientName={
                  selectedLeadId
                    ...
                    : isDemoAccount
                      ? displayMultiChairAppointments[0]?.patient || "Patient"
                      : "Select a patient"
                }
                onCheckIn={handleCheckInSelectedPatient}
                ...
                <Badge variant="outline" className="text-xs">
                  {
                    labOrders.filter(
                      (o: any) =>
                        o.status === "IN_PROGRESS" || o.status === "SENT",
                    ).length
                  }
                </Badge>
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-gray-600">
              Pre-built workflows for administrative tasks
            </p>
          </CardHeader>
          <CardContent>
