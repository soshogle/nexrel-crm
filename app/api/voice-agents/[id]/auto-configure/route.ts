...
export async function POST(
  ...
  try {
    ...
    if (!ctx) {
      console.log("❌ [Auto-Configure] Unauthorized - invalid session context");
      return apiErrors.unauthorized();
    }
    ...
    console.log(
      "🔍 [Auto-Configure] Fetching voice agent for user:",
      ctx.userId,
    );
    ...
    if (voiceAgent.userId !== ctx.userId) {
    ...
    if (voiceAgent.elevenLabsAgentId) {
      console.log(
        "🔍 [Auto-Configure] Agent has ID, validating in ElevenLabs:",
      ...
      if (validation.valid) {
        ...
        console.log(
          "🔧 [Auto-Configure] Agent exists, updating configuration to ensure completeness...",
        );

        const greeting =
          voiceAgent.type === "OUTBOUND"
            ? voiceAgent.outboundGreeting ||
              voiceAgent.greetingMessage ||
              `Hello! I'm calling from ${voiceAgent.businessName}. How can I help you today?`
            : voiceAgent.inboundGreeting ||
              voiceAgent.greetingMessage ||
              `Thank you for calling ${voiceAgent.businessName}. How can I help you today?`;

        const updateResult =
          await elevenLabsProvisioning.updateAgentConfiguration(
            voiceAgent.elevenLabsAgentId,
            {
              name: voiceAgent.name,
              businessName: voiceAgent.businessName,
              businessIndustry:
                voiceAgent.businessIndustry ||
                voiceAgent.user.industry ||
                "general business",
              greetingMessage: greeting,
              systemPrompt: voiceAgent.systemPrompt || undefined,
              knowledgeBase:
                voiceAgent.knowledgeBase ||
                voiceAgent.user.businessDescription ||
                undefined,
        ...
        if (!updateResult.success) {
          console.warn(
            "⚠️  [Auto-Configure] Failed to update agent configuration:",
            updateResult.error,
          );
        } else {
          console.log(
            "✅ [Auto-Configure] Agent configuration updated successfully",
          );
        }
        ...
        if (!validation.warnings?.some((w) => w.includes("No phone number"))) {
          ...
          console.warn(
            "⚠️  [Auto-Configure] Agent exists but missing phone number. Will re-import phone.",
          );
        ...
        console.warn(
          "⚠️  [Auto-Configure] Agent ID exists but agent not found in ElevenLabs. Will recreate.",
        );
    ...
    const reloadedAgent = await db.voiceAgent.findUnique({
      where: { id: voiceAgent.id },
      include: { user: true },
    });
    ...
    if (reloadedAgent.twilioPhoneNumber) {
      console.log(
        "🔍 [Auto-Configure] Checking if phone is already assigned to an agent...",
      );
      ...
      try {
        ...
        if (phoneResponse.ok) {
          ...
          const existingPhone = phoneNumbers.find(
            (p: any) =>
              p.phone_number?.replace(/\s/g, "") ===
              reloadedAgent.twilioPhoneNumber?.replace(/\s/g, ""),
          );

          if (existingPhone?.assigned_agent?.agent_id) {
            console.log(
              "✅ [Auto-Configure] Found existing agent with this phone!",
            );
            ...
            const agentNameMatch =
              existingPhone.assigned_agent.name === reloadedAgent.name;

            if (!agentNameMatch) {
              console.warn(
                "⚠️  [Auto-Configure] Agent name mismatch - this phone is assigned to a different agent",
              );
              console.warn(
                `   Expected: "${reloadedAgent.name}", Found: "${existingPhone.assigned_agent.name}"`,
              );
              console.warn(
                "   This may be a duplicate agent. Creating a new agent instead...",
              );
              ...
              console.log(
                "🔗 [Auto-Configure] Re-assigning phone to ensure proper linkage...",
              );
              ...
              if (assignResponse.ok) {
                console.log(
                  "✅ [Auto-Configure] Phone re-assigned successfully",
                );
              } else {
                console.warn(
                  "⚠️  [Auto-Configure] Phone re-assignment failed, but continuing...",
                );
              }
        ...
        console.warn(
          "⚠️  [Auto-Configure] Could not check for existing agents:",
          error,
        );
    ...
    if (reloadedAgent.elevenLabsAgentId) {
      console.log(
        "🔗 [Auto-Configure] Agent exists, importing phone number to:",
      ...
      if (reloadedAgent.twilioPhoneNumber) {
        ...
        if (phoneResult.success) {
          console.log(
            "✅ [Auto-Configure] Phone imported and assigned:",
          ...
          if (assignResponse.ok) {
            ...
            console.warn(
              "⚠️  [Auto-Configure] Phone re-assignment may have failed",
            );
          }
          ...
          console.error(
            "❌ [Auto-Configure] Phone import failed:",
            phoneResult.error,
          );
          return NextResponse.json(
            {
      ...
      console.log(
        "🤖 [Auto-Configure] Creating new ElevenLabs agent for:",
        reloadedAgent.name,
      );

      // Determine which greeting to use based on agent type
      const greetingForElevenLabs =
        reloadedAgent.type === "OUTBOUND"
          ? reloadedAgent.outboundGreeting ||
            reloadedAgent.greetingMessage ||
            `Hello! I'm calling from ${reloadedAgent.businessName}. How can I help you today?`
          : reloadedAgent.inboundGreeting ||
            reloadedAgent.greetingMessage ||
            `Thank you for calling ${reloadedAgent.businessName}. How can I help you today?`;

      result = await elevenLabsProvisioning.createAgent({
        name: reloadedAgent.name,
        businessName: reloadedAgent.businessName,
        businessIndustry:
          reloadedAgent.businessIndustry ||
          reloadedAgent.user.industry ||
          "general business",
        greetingMessage: greetingForElevenLabs,
        systemPrompt: reloadedAgent.systemPrompt || undefined,
        knowledgeBase:
          reloadedAgent.knowledgeBase ||
          reloadedAgent.user.businessDescription ||
          undefined,
      ...
      if (!result.success) {
        console.log(
          "❌ [Auto-Configure] ElevenLabs provisioning failed:",
          result.error,
        );
        return NextResponse.json(
          {
      ...
      console.log(
        "✅ [Auto-Configure] ElevenLabs agent created:",
        result.agentId,
      );
