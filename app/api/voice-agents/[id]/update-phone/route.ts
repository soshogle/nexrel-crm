...
export async function PATCH(
  ...
  try {
    ...
    if (agent.elevenLabsAgentId) {
      try {
        console.log(
          `📞 Importing phone ${phoneNumber} into ElevenLabs for agent ${agent.name}`,
        );
        ...
        if (importResult.success && importResult.phoneNumberId) {
          ...
          console.log(
            "✅ Phone number successfully imported and assigned in ElevenLabs",
          );
        } else {
      ...
      console.warn(
        "⚠️ Voice agent has no ElevenLabs ID - skipping ElevenLabs import",
      );
      elevenLabsError =
        "No Soshogle Voice AI agent ID configured. Please configure the agent first.";
    }
    ...
    console.log(
      `Phone number ${phoneNumber} assigned to voice agent ${updatedAgent.name}`,
    );
