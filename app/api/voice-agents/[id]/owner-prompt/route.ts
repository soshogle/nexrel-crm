...
export async function POST(
  ...
  try {
    ...
    const addition =
      typeof body?.addition === "string" ? body.addition.trim() : "";
    ...
    if (!agent.elevenLabsAgentId) {
      return apiErrors.badRequest(
        "Agent has no Soshogle Voice ID; create or provision first",
      );
    }
    ...
    if (getRes.ok) {
      ...
      if (delimIdx >= 0) {
        ...
        previousOwnerSection =
          currentPrompt
            .slice(delimIdx + OWNER_CUSTOMIZATION_DELIMITER.length)
            .trim() || null;
      } else {
        basePrompt =
          currentPrompt ||
          `You are a helpful voice assistant for ${agent.businessName}. Answer questions professionally and helpfully.`;
      }
    } else {
    ...
    fullPrompt =
      fullPrompt + EASTERN_TIME_SYSTEM_INSTRUCTION + getConfidentialityGuard();
    ...
    if (!result.success) {
      return apiErrors.internal(
        result.error || "Failed to update Soshogle Voice agent",
      );
    }
