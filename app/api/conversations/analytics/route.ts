...
export async function GET(req: NextRequest) {
  try {
    ...
    const avgDuration =
      calls.reduce((sum, c) => sum + (c.duration || 0), 0) / (totalCalls || 1);
    ...
    const outcomeCounts = calls.reduce(
      (acc, call) => {
    ...
    if (avgDuration < 60000) {
      insights.push(
        "Average call duration is low - consider engagement strategies",
      );
    }
