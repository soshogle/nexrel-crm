...
async function getUserUsageStats(userId: string, startDate: Date) {
  ...
  const callLogs = await getMetaDb().callLog.findMany({
    where: {
      userId,
  ...
  const totalCallMinutes =
    callLogs.reduce((sum, c) => sum + (c.duration || 0), 0) / 60;
  const inboundCalls = callLogs.filter((c) => c.direction === "INBOUND").length;
  const outboundCalls = callLogs.filter(
  ...
  // SMS Statistics
  const smsMessages = await getMetaDb().conversationMessage.findMany({
    where: {
      conversation: {
        userId,
        channelConnection: {
  ...
  const inboundSMS = smsMessages.filter(
    (m) => m.direction === "INBOUND",
  ).length;
  const outboundSMS = smsMessages.filter(
  ...
  // Email Statistics
  const emailMessages = await getMetaDb().conversationMessage.findMany({
    where: {
      conversation: {
        userId,
        channelConnection: {
  ...
  const inboundEmails = emailMessages.filter(
    (m) => m.direction === "INBOUND",
  ).length;
  const outboundEmails = emailMessages.filter(
    (m) => m.direction === "OUTBOUND",
  ).length;
  ...
  const aiConversations = await getMetaDb().conversationMessage.findMany({
    where: {
      conversation: {
        userId,
      },
  ...
  // Voice Agent Usage
  const voiceAgentCount = await getMetaDb().voiceAgent.count({
    where: { userId },
  });

  // Storage Usage (approximate)
  const recordingsCount = callLogs.filter(
    (c) => c.status === "COMPLETED",
  ).length;
  ...
  // Total Costs
  const totalCost =
    callCosts + smsCosts + emailCosts + aiTokenCosts + storageCosts;
