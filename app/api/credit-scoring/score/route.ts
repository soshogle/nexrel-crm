...
export async function POST(req: NextRequest) {
  try {
    ...
    const [existingScore, applications] = await Promise.all([
      db.creditScore.findUnique({ where: { userId: session.user.id } }),
    ...
    const deniedApps = applications.filter(
      (a) => a.status === "DENIED" || a.status === "EXPIRED",
    ).length;
      ...
      const utilization =
        existingScore && existingScore.creditLimit > 0
          ? Math.max(
              ...
              Math.min(
                1,
                1 - existingScore.availableCredit / existingScore.creditLimit,
              ),
            )
          : 0.5;
