...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "📄 Uploading knowledge base document for user:",
      session.user.id,
    );
    ...
    if (!agentIdToLink) {
      ...
      let draftAgent = await db.voiceAgent.findFirst({
        where: {
      ...
      if (!draftAgent) {
        ...
        draftAgent = await db.voiceAgent.create({
          data: {
            userId: session.user.id,
            name: "Draft Agent",
            description:
              "Automatically created during onboarding. Files uploaded during onboarding are linked here.",
            type: "INBOUND",
            status: "TESTING",
            businessName: "Draft",
          },
    ...
    const knowledgeBaseFile = await db.knowledgeBaseFile.create({
      data: {
        userId: session.user.id,
        fileName: file.name,
    ...
    if (agentIdToLink) {
      await db.voiceAgentKnowledgeBaseFile.create({
        data: {
