  ...
  try {
    ...
    if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN) {
      ...
      return NextResponse.json(
        { error: 'Twilio credentials not configured', details: 'Please set TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN' },
        { status: 500 }
      );
    }
    ...
    if (!ELEVENLABS_API_KEY) {
      ...
      return NextResponse.json(
        { error: 'ElevenLabs API key not configured', details: 'Please set ELEVENLABS_API_KEY' },
        { status: 500 }
      );
    }
    ...
    if (!subscriptionCheck.canUsePhoneNumbers) {
      console.error('[phone-sync] ElevenLabs plan does not support phone numbers');
      return NextResponse.json(
        {
    ...
    for (const twilioNumber of twilioNumbers) {
      ...
      try {
        ...
        const importResult = await elevenLabsProvisioning.importPhoneNumber(phoneNumber, '', user.id);

        if (importResult.success) {
          results.importedToElevenLabs++;
        } else {
          console.error('[phone-sync] ElevenLabs import failed for', phoneNumber, ':', importResult.error);
        ...
        console.error('[phone-sync] Error processing', phoneNumber, ':', error.message);
