...
function getCandidateDbs() {
  ...
  for (const key of Object.keys(process.env)) {
    ...
    try {
      ...
      // Ignore invalid/missing industry DB clients
    }
  }
...
async function findCallLogByTwilioSid(callSid: string) {
  for (const db of getCandidateDbs()) {
  ...
  try {
    ...
    if (recordingStatus !== "completed") {
      return NextResponse.json({
        success: true,
        message: "Recording not completed yet",
      });
    }

    const located = await findCallLogByTwilioSid(callSid);
    const callLog = located?.callLog;
    const db = located?.db;

    if (!callLog || !db) {
      console.error(
        "[recording-status] Call log not found for CallSid:",
        callSid,
      );
    ...
    if (voiceAgent.enableTranscription && recordingSid) {
      try {
        ...
        console.error(
          "[recording-status] Error fetching transcription:",
          transcriptionError,
        );
      ...
      try {
        ...
        await emailService.sendCallSummaryEmail({
          ...
          callDuration: callLog.duration
            ? `${Math.ceil(callLog.duration / 60)} min`
            : "N/A",
        ...
        console.error(
          "[recording-status] Error sending recording email:",
          emailError,
        );
      }
    }
...
async function fetchTwilioTranscription(recordingSid: string): Promise<string> {
  try {
    ...
    console.error(
      "[recording-status] Error fetching Twilio transcription:",
      error,
    );
    return "";
  }
