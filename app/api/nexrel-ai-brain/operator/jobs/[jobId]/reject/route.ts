...
import { rejectNexrelAiBrainJob } from "@/lib/nexrel-ai-brain/operator";
import {
  ...
  try {
    ...
    if (idempotencyKey) {
      const cached = getIdempotentResponse(
        idempotencyKey,
        `nexrel-ai-brain-reject:${params.jobId}:${ctx.userId}`,
      );
      if (cached) {
    ...
    console.error("[nexrel-ai-brain] reject job error", error);
    return apiErrors.internal(
      error?.message || "Failed to reject operator job",
    );
  }
}
