  ...
  try {
    ...
    if (!user?.industry || user.industry === "REAL_ESTATE") {
      return apiErrors.forbidden(
        "This feature is not available for this industry",
      );
    }
    ...
    if (!industryConfig) {
      return apiErrors.forbidden(
        "Workflow system not configured for this industry",
      );
    }
    ...
    if (templateId) {
      ...
      const template = industryConfig.templates.find(
        (t) => t.id === templateId,
      );
      if (!template) {
      ...
      const transformedTemplate = {
        ...
        tasks: template.tasks.map((task, index) => ({
          ...
          agentColor:
            industryConfig.aiAgents.find((a) => a.name === task.agentName)
              ?.color || "#6B7280",
    ...
    const templates = industryConfig.templates.map((template) => ({
      ...
      tags: [
        template.workflowType.toLowerCase().replace(/_/g, "-"),
        "automation",
      ],
      icon: "⚡", // Default icon
      ...
      agentsAssigned: new Set(
        template.tasks.map((t) => t.agentName).filter(Boolean),
      ).size,
    }));
