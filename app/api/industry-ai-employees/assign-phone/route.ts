  ...
  try {
    ...
    if (!industry || !employeeType || !phoneNumber) {
      return apiErrors.badRequest(
        "industry, employeeType, and phoneNumber are required",
      );
    }

    const agent = await db.industryAIEmployeeAgent.findUnique({
      where: {
    ...
    if (!agent) {
      return apiErrors.notFound(
        "Industry AI agent not provisioned. Please provision this agent first.",
      );
    }
    ...
    await db.industryAIEmployeeAgent.update({
      where: {
        userId_industry_employeeType: {
          userId: session.user.id,
          industry,
