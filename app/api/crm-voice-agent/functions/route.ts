  ...
  try {
    ...
    if (
      ...
      try {
        ...
        console.error(
          "[CRM Voice Functions] Failed to log Nexrel AI operation",
          logError,
        );
      }
    }
...
async function handleNexrelAiOperate(
    ...
    const taskResult = await proxyToActionsAPI(
      "create_task",
      {
        ...
        description:
          ...
          `Nexrel AI chain follow-up for ${contactName}`,
    ...
    for (const lead of leads as any[]) {
      await (db as any).task.create({
        data: {
          ...
          description: "Generated by Nexrel AI sales_squad mode.",
          priority: "HIGH",
          status: "TODO",
          autoCreated: true,
          aiSuggested: true,
  ...
  if (mode === "vertical_playbook") {
    ...
    const workflowName =
      ...
      (vertical === "real_estate"
        ? "Nexrel AI Real Estate Nurture Playbook"
        : "Nexrel AI Dental Recall & Follow-up Playbook");
  ...
  if (mode === "customer_voice_workflow") {
    const workflowName =
      params?.workflowName || "Nexrel AI Customer Voice Qualification Flow";
    const createWorkflow = await proxyToActionsAPI(
      "create_workflow",
      {
        name: workflowName,
