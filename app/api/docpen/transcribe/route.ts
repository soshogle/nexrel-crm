...
} from "@/lib/docpen/transcription-service";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) {
      console.error(
        "[Docpen Transcribe] ❌ OPENAI_API_KEY not found in environment",
      );
      return NextResponse.json(
        {
          error: "API key not configured",
          details:
            "OPENAI_API_KEY environment variable is not set. Please add it to your Vercel environment variables.",
    ...
    console.log(
      "[Docpen Transcribe] ✅ API key found (first 10 chars):",
    ...
    try {
      ...
      if (
        errorMessage.includes("Abacus") ||
        errorMessage.includes("ABACUS") ||
        errorMessage.includes("initialize LLM APIs")
      ) {
        errorMessage =
          "OpenAI API key not configured. Please add OPENAI_API_KEY to your Vercel environment variables.";
        console.warn(
          "[Docpen Transcribe] ⚠️ Old error format detected - this indicates old code is running",
        );
      }
