...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existing) {
      ...
      const conversation = await db.docpenConversation.update({
    ...
    const conversation = await db.docpenConversation.create({
      data: {
        agentId,
    ...
    return apiErrors.internal(error.message || "Failed to save conversation");
  }
}
