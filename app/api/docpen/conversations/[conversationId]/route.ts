...
export async function GET(
  ...
  try {
    ...
    // Get agent info
    const agentInfo = docpenAgents.find(
      (a) => a.elevenLabsAgentId === conversation.agent_id,
    );
    const agentName =
  ...
  try {
    ...
    if (!isSuperAdmin) {
      return apiErrors.forbidden(
        "Forbidden: Only super admins can delete conversations",
      );
    }
    ...
    const deletedConv = await db.docpenConversation.deleteMany({
      where: {
