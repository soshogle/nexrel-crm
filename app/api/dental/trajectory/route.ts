...
export async function GET(request: NextRequest) {
  try {
    ...
    const [perioCharts, odontograms, xrays, treatmentPlans, recalls] = await Promise.all([
    ...
    const perioExams: PerioExam[] = perioCharts.map((c: any) => ({
      ...
      measurements: (typeof c.measurements === 'object' ? c.measurements : {}) as Record<string, any>,
    }));

    const odontogramSnapshots: OdontogramSnapshot[] = odontograms.map((o: any) => ({
      date: (o.updatedAt || o.createdAt).toISOString(),
      data: (typeof o.toothData === 'object' ? o.toothData : {}) as Record<string, any>,
    }));

    const xrayFindings: XrayFinding[] = xrays.map((x: any) => {
      const analysis = typeof x.aiAnalysis === 'object' ? x.aiAnalysis as any : {};
      return {
        id: x.id,
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to compute trajectory' },
      { status: 500 },
    );
  }
}
