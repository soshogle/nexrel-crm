...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!leadId && conversationId) {
      const conv = await conversationService.findFirst(ctx, {
        id: conversationId,
      });
    ...
    if (lead) {
      ...
      if (
        nextTask?.dueDate &&
        new Date(nextTask.dueDate) <= new Date(Date.now() + 24 * 60 * 60 * 1000)
      ) {
        contextReplies.push({
          id: "task_reminder",
          label: "Task reminder",
      ...
      if (
        lastNote?.content?.toLowerCase().includes("documents") ||
        lastNote?.content?.toLowerCase().includes("materials")
      ) {
        contextReplies.push({
          id: "docs_ready",
