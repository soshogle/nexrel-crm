...
export async function GET(request: NextRequest) {
  try {
    ...
    const rules = await getCrmDb(ctx).taskAutomation.findMany({
  ...
  try {
    ...
    if (!name || !triggerType || !actionType) {
      return apiErrors.badRequest(
        "Missing required fields: name, triggerType, actionType",
      );
    }

    const rule = await getCrmDb(ctx).taskAutomation.create({
      data: {
