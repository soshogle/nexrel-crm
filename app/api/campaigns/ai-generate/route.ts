...
export async function POST(request: NextRequest) {
  try {
    ...
    const { campaignType, goal, targetAudience, tone, includePersonalization } =
      body;

    // Validation
    if (
      !campaignType ||
      !["EMAIL", "SMS", "VOICE_CALL", "MULTI_CHANNEL"].includes(campaignType)
    ) {
      return apiErrors.badRequest(
        "Valid campaign type is required (EMAIL, SMS, VOICE_CALL, or MULTI_CHANNEL)",
      );
    }
    ...
    if (campaignType === "EMAIL" || campaignType === "MULTI_CHANNEL") {
      const emailContent =
        await aiCampaignService.generateEmailContent(context);
      generatedContent.email = emailContent;
      generatedContent.emailSubjects =
        await aiCampaignService.generateEmailSubjects(context);
    }
    ...
    if (campaignType === "VOICE_CALL" || campaignType === "MULTI_CHANNEL") {
      const voiceContent =
        await aiCampaignService.generateVoiceCallScript(context);
