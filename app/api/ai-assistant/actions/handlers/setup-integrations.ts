...
export async function setupStripe(userId: string, params: any) {
  ...
  return {
    message:
      "✅ Stripe has been successfully configured! You can now accept payments through Stripe.",
    provider: "Stripe",
...
export async function setupSquare(userId: string, params: any) {
  ...
  return {
    message:
      "✅ Square has been successfully configured! You can now accept payments through Square.",
    provider: "Square",
    nextSteps: [
      "Create payment links",
      "Set up invoicing",
...
export async function setupPayPal(userId: string, params: any) {
  ...
  return {
    message:
      "✅ PayPal has been successfully configured! You can now accept payments through PayPal.",
    provider: "PayPal",
    nextSteps: [
...
export async function createQuickBooksInvoice(userId: string, params: any) {
  ...
  const response = await fetch(
    `${process.env.NEXTAUTH_URL}/api/integrations/quickbooks/invoice`,
    {
      method: "POST",
...
export async function syncContactToQuickBooks(userId: string, params: any) {
  ...
  const response = await fetch(
    `${process.env.NEXTAUTH_URL}/api/integrations/quickbooks/sync-contact`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
