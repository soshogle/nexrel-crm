...
export async function getStatistics(userId: string, params: any = {}) {
  ...
  try {
    ...
    const [leads, deals, contacts, campaigns, additionalStats] =
      await Promise.all([
    ...
    if (scenarioIntent) {
      const { parseScenarioIntent, calculateScenario } = await import(
        "@/lib/crm-scenario-predictor"
      );
    ...
    if (chartIntent) {
      const { parseChartIntent, getDynamicChartData } = await import(
        "@/lib/crm-chart-intent"
      );
      ...
      if (intent) {
        ...
        if (intent.dimension === "revenue_by_month") {
          ...
          data = await getDynamicChartData(
            userId,
            intent.dimension,
            ctx.industry,
          );
  ...
  try {
    ...
    emailCampaigns.forEach((c: any) => {
      (c.recipients || []).forEach((r: any) => {
        if (["SENT", "DELIVERED", "OPENED", "CLICKED"].includes(r.status))
          emailSent++;
    ...
    const smsReplyRate =
      smsDelivered > 0 ? (smsReplied / smsDelivered) * 100 : 0;

    // Drip campaign stats
    const activeDrips = dripCampaigns.filter(
      (c: any) => c.status === "ACTIVE",
    ).length;
    const avgDripOpen =
      dripCampaigns.length > 0
        ? dripCampaigns.reduce(
            (s: number, c: any) => s + (c.avgOpenRate || 0),
            0,
    ...
    const answerRate =
      totalCalls > 0 ? (completedCalls.length / totalCalls) * 100 : 0;
    const avgDuration =
      completedCalls.length > 0
        ? completedCalls.reduce(
            (s: number, c: any) => s + (c.duration || 0),
            0,
    ...
    const scores = leadsWithScores
      .map((l: any) => l.leadScore)
    ...
    return {
      campaignPerformance: {
        ...
        smsReplied,
        smsReplyRate,
        activeDripCampaigns: activeDrips,
        avgDripOpenRate: avgDripOpen,
      ...
      voiceCallAnalytics: {
        ...
        answerRate,
        avgDuration,
        outboundScheduled: outboundCalls.length,
        outboundCompleted,
      },
      workflowMetrics: {
        activeWorkflows,
...
export async function createReport(userId: string, params: any) {
  ...
  const statsPeriod = ["last_7_days", "last_month"].includes(period)
    ? "last_30_days"
    : period;
  ...
  const agentStats = voiceAgents
    .map((a) => ({
      ...
      totalUsage:
        a._count.callLogs +
        a._count.campaigns * 10 +
        (aiCountByAgent[a.id] || 0) * 5, // Weighted score for "most used"
    }))
  ...
  const content: any = {
    summary:
      statsResult.message ||
      `Report generated for ${period}. You have ${stats.totalLeads} leads, ${stats.totalDeals} deals, ${stats.openDeals} open deals worth $${(stats.totalRevenue || 0).toLocaleString()}, and ${stats.totalCampaigns} campaigns.`,
    metrics: {
  ...
  if (
    stats.marketStats &&
    (stats.marketStats.medianPrice != null ||
      stats.marketStats.salesVolume != null)
  ) {
...
export async function getCustomReport(userId: string, params: any) {
  ...
  if (!intent) {
    return {
      message:
        "Could not parse report query. Try: 'leads by source', 'revenue by month', 'deals by stage'.",
      data: [],
    };
  }
  const data = await getDynamicChartData(
    userId,
    intent.dimension,
    ctx.industry,
  );
...
export async function getFollowUpPriority(userId: string, params: any) {
  ...
  const scored = leads.map((l: any) => {
    ...
    if (
      !lastContact ||
      (lastContact &&
        Date.now() - lastContact.getTime() > 7 * 24 * 60 * 60 * 1000)
    )
      score += 30;
...
export async function getFollowUpSuggestions(userId: string, params: any) {
  ...
  const daysAgo =
    period === "last_week" ? 7 : period === "last_2_weeks" ? 14 : 30;
...
export async function getMeetingPrep(userId: string, params: any) {
  ...
  const briefing = {
    ...
    recentNotes: lead.notes.map((n: any) => ({
      content: n.content,
      date: n.createdAt,
    })),
    deals: lead.deals.map((d: any) => ({
      title: d.title,
      value: d.value,
      stage: d.stage?.name,
    })),
    openTasks: lead.tasks.map((t: any) => ({
      title: t.title,
      dueDate: t.dueDate,
    })),
...
export async function getDailyBriefing(userId: string) {
  ...
  if (overdueTasks.length > 0) {
    summary.push(
      `${overdueTasks.length} overdue task(s): ${overdueTasks.map((t) => t.title).join(", ")}`,
    );
  }
  if (todayTasks.length > 0) {
    summary.push(
      `${todayTasks.length} task(s) due today: ${todayTasks.map((t) => t.title).join(", ")}`,
    );
  }
  if (appointments.length > 0) {
    summary.push(
      `${appointments.length} appointment(s) today: ${appointments.map((a) => a.customerName).join(", ")}`,
    );
  }
  if (overdueInvoices.length > 0) {
    summary.push(
      `${overdueInvoices.length} overdue invoice(s): ${overdueInvoices.map((i) => `${i.customerName} ($${i.totalAmount})`).join(", ")}`,
    );
  }
  if (newLeads.length > 0) {
    summary.push(
      `${newLeads.length} new lead(s) this week: ${newLeads.map((l) => l.contactPerson || l.businessName).join(", ")}`,
    );
  }
  if (hotDeals.length > 0) {
    summary.push(
      `Top deals: ${hotDeals.map((d) => `${d.title} ($${d.value})`).join(", ")}`,
    );
  }
  if (summary.length === 0) {
...
export async function getAutoActionSuggestions(userId: string, params: any) {
  ...
  const realtime = await aiBrainService.detectRealtimePatterns(userId);
  const combined = [...realtime, ...insights].filter(
    (i) => i.actionable && i.suggestedActions?.length,
  );
  ...
  return {
    success: true,
    statistics: { totalSuggestions: actions.length, suggestions: actions },
    message: `${actions.length} action suggestions: ${actions.map((a, i) => `${i + 1}. [${a.priority.toUpperCase()}] ${a.title} → ${a.suggestedAction}`).join("; ")}`,
  };
}
...
export async function getPaymentAnalytics(userId: string, params: any) {
  ...
  const [payments, invoices, cashTxns, fraudAlerts] = await Promise.all([
    ...
    (db as any).cashTransaction
      .findMany({
        where: { userId: ctx.userId, createdAt: { gte: since } },
        select: { amount: true, type: true },
      })
      .catch(() => []),
    (db as any).fraudAlert
      .count({ where: { userId: ctx.userId, status: "OPEN" } })
      .catch(() => 0),
  ]);
  ...
  succeeded.forEach((p: any) => {
    const m = p.paymentMethod || "unknown";
    methodBreakdown[m] = (methodBreakdown[m] || 0) + (p.amount || 0);
  });
    ...
    dynamicCharts: [
      {
        ...
        data: Object.entries(methodBreakdown).map(([name, value]) => ({
          name,
...
export async function getRevenueBreakdown(userId: string, params: any) {
    ...
    payments.forEach((p: any) => {
      ...
      const key =
        groupBy === "month"
          ...
          : `W${Math.ceil((d.getTime() - since.getTime()) / 86400000 / 7)}`;
      buckets[key] = (buckets[key] || 0) + (p.amount || 0);
    });
    return {
      success: true,
      triggerVisualization: true,
      statistics: { breakdown: buckets },
      message: `Revenue breakdown by ${groupBy}: ${Object.entries(buckets)
        .map(
          ([k, v]) =>
            `${k}: $${Math.round((v as number) / 100).toLocaleString()}`,
        )
        .join(", ")}`,
      dynamicCharts: [
        {
          chartType: "line",
          dimension: `revenue_by_${groupBy}`,
          title: `Revenue by ${groupBy}`,
          data: Object.entries(buckets).map(([name, value]) => ({
            name,
            value: Math.round((value as number) / 100),
          })),
        },
  ...
  payments.forEach((p: any) => {
    ...
    methodBuckets[m] = (methodBuckets[m] || 0) + (p.amount || 0);
  });
  return {
    success: true,
    triggerVisualization: true,
    statistics: { breakdown: methodBuckets },
    message: `Revenue by method: ${Object.entries(methodBuckets)
      .map(
        ([k, v]) =>
          `${k}: $${Math.round((v as number) / 100).toLocaleString()}`,
      )
    ...
    dynamicCharts: [
      {
        chartType: "pie",
        dimension: "revenue_by_method",
        title: "Revenue by Payment Method",
        data: Object.entries(methodBuckets).map(([name, value]) => ({
          name,
          value: Math.round((value as number) / 100),
        })),
      },
...
export async function listFraudAlerts(userId: string, params: any) {
  const statusFilter =
    params?.status === "all" ? undefined : params?.status || "OPEN";
  const ctx = await resolveDalContext(userId);
  const db = getCrmDb(ctx);
  const where: any = { userId: ctx.userId };
  if (statusFilter) where.status = statusFilter;
  const alerts = await (db as any).fraudAlert
    .findMany({ where, orderBy: { createdAt: "desc" }, take: 20 })
    .catch(() => []);
  return {
    success: true,
    statistics: {
    ...
    message: `${alerts.length} fraud alert(s) found${statusFilter ? ` with status ${statusFilter}` : ""}.`,
  };
}
...
export async function checkCashFlow(userId: string, params: any) {
  ...
  const [incoming, outgoing] = await Promise.all([
    ...
    db.invoice.findMany({
      where: { userId: ctx.userId, status: "PAID", createdAt: { gte: since } },
  ...
  const totalOut = outgoing.reduce(
    (s: number, i: any) => s + (i.totalAmount || 0),
    0,
  );
  ...
  return {
    ...
    message: `Cash flow (last ${days} days): $${(totalIn / 100).toLocaleString()} in, $${(totalOut / 100).toLocaleString()} out. Net: $${(net / 100).toLocaleString()}. ${net > 0 ? "Positive cash flow." : "Negative cash flow — review expenses."}`,
    dynamicCharts: [
      {
        chartType: "bar",
        dimension: "cash_flow",
        title: "Cash Flow",
        data: [
          { name: "Incoming", value: Math.round(totalIn / 100) },
          { name: "Outgoing", value: Math.round(totalOut / 100) },
          { name: "Net", value: Math.round(net / 100) },
        ],
      },
    ],
  };
...
export async function checkStockLevels(userId: string, params: any) {
  const ctx = await resolveDalContext(userId);
  const db = getCrmDb(ctx);
  const items = await (db as any).inventoryItem
    .findMany({
      where: { userId: ctx.userId },
      select: {
        id: true,
  ...
  if (params?.filter === "low_stock")
    filtered = items.filter(
      (i: any) => i.quantity <= (i.minQuantity || 5) && i.quantity > 0,
    );
  else if (params?.filter === "out_of_stock")
    filtered = items.filter((i: any) => i.quantity === 0);
  else if (params?.filter === "overstocked")
    filtered = items.filter((i: any) => i.quantity > (i.minQuantity || 5) * 3);
  const limit = params?.limit || 20;
  return {
    success: true,
    statistics: {
      total: items.length,
      lowStock: items.filter((i: any) => i.quantity <= (i.minQuantity || 5))
        .length,
    ...
    message: `${items.length} inventory items. ${items.filter((i: any) => i.quantity <= (i.minQuantity || 5)).length} low stock, ${items.filter((i: any) => i.quantity === 0).length} out of stock.`,
  };
}
...
export async function getBestSellers(userId: string, params: any) {
  ...
  const ctx = await resolveDalContext(userId);
  const db = getCrmDb(ctx);
  const orders = await (db as any).orderItem
    .findMany({
  ...
  orders.forEach((oi: any) => {
    const n = oi.product?.name || "Unknown";
    if (!productMap[n]) productMap[n] = { name: n, revenue: 0, qty: 0 };
    productMap[n].revenue += (oi.price || 0) * (oi.quantity || 1);
    productMap[n].qty += oi.quantity || 1;
  });
  const sorted = Object.values(productMap).sort((a, b) =>
    params?.sortBy === "quantity" ? b.qty - a.qty : b.revenue - a.revenue,
  );
  const top = sorted.slice(0, params?.limit || 10);
  return {
    success: true,
    triggerVisualization: true,
    statistics: { bestSellers: top },
    message: `Top ${top.length} best sellers: ${top.map((p, i) => `${i + 1}. ${p.name} ($${p.revenue.toLocaleString()}, ${p.qty} sold)`).join("; ")}`,
    dynamicCharts: [
      {
        chartType: "bar",
        dimension: "best_sellers",
        title: "Best Selling Products",
        data: top.map((p) => ({
          name: p.name,
          value: params?.sortBy === "quantity" ? p.qty : Math.round(p.revenue),
        })),
      },
    ],
  };
...
export async function trackOrder(userId: string, params: any) {
  ...
  let order: any = null;
  if (params?.orderId)
    order = await (db as any).order
      .findFirst({
        where: { userId: ctx.userId, id: params.orderId },
        include: {
          items: { include: { product: { select: { name: true } } } },
        },
      })
      .catch(() => null);
  if (!order && params?.customerName)
    order = await (db as any).order
      .findFirst({
        where: {
          userId: ctx.userId,
  ...
  return {
    ...
    message: `Order ${order.id}: Status ${order.status}, Total $${(order.total || 0).toLocaleString()}, ${order.items?.length || 0} items. Created ${new Date(order.createdAt).toLocaleDateString()}.`,
  };
}
...
export async function getLowStockAlerts(userId: string) {
  ...
  const [items, alerts] = await Promise.all([
    (db as any).inventoryItem
      .findMany({
        where: { userId: ctx.userId },
        select: { name: true, quantity: true, minQuantity: true },
      })
      .catch(() => []),
    (db as any).inventoryAlert
      .findMany({
  ...
  return {
    ...
    message: `${lowStock.length} items need restocking: ${lowStock
      .slice(0, 5)
      .map((i: any) => `${i.name} (${i.quantity}/${i.minQuantity})`)
      .join(
        ", ",
...
export async function getWebsiteAnalytics(userId: string, params: any) {
  ...
  const live = websites.filter(
    (w: any) => w.status === "LIVE" || w.status === "DEPLOYED",
  );
  return {
    success: true,
    statistics: {
      totalWebsites: websites.length,
      liveWebsites: live.length,
    ...
    message: `${websites.length} website(s): ${live.length} live, ${websites.length - live.length} in progress. ${live.map((w: any) => w.name).join(", ") || "None live yet."}`,
  };
}
...
export async function getVoiceAIAnalytics(userId: string, params: any) {
  ...
  const [agents, usage, calls] = await Promise.all([
    db.voiceAgent.findMany({
      where: { userId: ctx.userId },
      select: { name: true, status: true, totalCalls: true },
    } as any),
    (db as any).voiceUsage
      .findMany({
        where: { userId: ctx.userId, createdAt: { gte: since } },
        select: { minutes: true, cost: true },
      })
      .catch(() => []),
    db.callLog.findMany({
      where: { userId: ctx.userId, createdAt: { gte: since } },
  ...
  const avgDuration =
    completed.length > 0
      ? Math.round(
          completed.reduce((s: number, c: any) => s + (c.duration || 0), 0) /
            completed.length,
        )
      : 0;
  return {
    ...
    statistics: {
      ...
      activeAgents: agents.filter(
        (a: any) => a.status === "ACTIVE" || a.status === "active",
      ).length,
      ...
      costPerMinute:
        totalMin > 0 ? Math.round((totalCost / totalMin) * 100) / 100 : 0,
    },
    message: `${agents.length} voice agent(s), ${calls.length} calls (${answerRate}% answer rate, avg ${avgDuration}s). Usage: ${Math.round(totalMin)} min, $${totalCost.toFixed(2)} cost.`,
    dynamicCharts: [
      {
        chartType: "bar",
        dimension: "voice_metrics",
        title: "Voice AI Metrics",
        data: [
          { name: "Total Calls", value: calls.length },
          { name: "Completed", value: completed.length },
          { name: "Answer Rate %", value: answerRate },
        ],
      },
    ],
  };
...
export async function getConversationAnalytics(userId: string, params: any) {
  ...
  const [conversations, messages] = await Promise.all([
    ...
    db.conversationMessage.findMany({
      where: { conversation: { userId: ctx.userId }, sentAt: { gte: since } },
      select: { direction: true, channel: true, sentAt: true },
    } as any),
  ]);
  ...
  return {
    ...
    triggerVisualization: true,
    statistics: {
      activeConversations: conversations.filter((c: any) => c.status === "OPEN")
        .length,
    ...
    message: `${conversations.length} conversations, ${messages.length} messages in ${days}d (${inbound} inbound, ${outbound} outbound). Channels: ${Object.entries(
      channelCounts,
    )
      .map(([k, v]) => `${k}: ${v}`)
      .join(", ")}.`,
    dynamicCharts: [
      {
        chartType: "pie",
        dimension: "messages_by_channel",
        title: "Messages by Channel",
        data: Object.entries(channelCounts).map(([name, value]) => ({
          name,
          value,
        })),
      },
...
export async function getDeliveryAnalytics(userId: string) {
  ...
  const pending = orders.filter(
    (o: any) => o.status === "PENDING" || o.status === "IN_TRANSIT",
  ).length;
  return {
    success: true,
    statistics: {
    ...
    message: `${orders.length} delivery orders: ${delivered} delivered, ${pending} pending. Completion rate: ${orders.length > 0 ? Math.round((delivered / orders.length) * 100) : 0}%.`,
  };
}
...
export async function manageReservations(userId: string, params: any) {
  ...
  if (action === "stats" || action === "peak_hours") {
    const reservations = await (db as any).reservation
      .findMany({
        where: { userId: ctx.userId },
    ...
    return {
      ...
      statistics: {
        ...
        noShowRate:
          reservations.length > 0
            ? Math.round((noShows / reservations.length) * 100)
            : 0,
        avgPartySize:
          reservations.length > 0
            ? Math.round(
                reservations.reduce(
                  (s: number, r: any) => s + (r.partySize || 0),
                  0,
                ) / reservations.length,
              )
            : 0,
      ...
      message: `${reservations.length} total reservations. No-show rate: ${reservations.length > 0 ? Math.round((noShows / reservations.length) * 100) : 0}%. Peak hour: ${peakHour ? `${peakHour[0]}:00` : "N/A"}. Avg party: ${reservations.length > 0 ? Math.round(reservations.reduce((s: number, r: any) => s + (r.partySize || 0), 0) / reservations.length) : 0}.`,
      dynamicCharts: [
        {
          chartType: "bar",
          dimension: "reservations_by_hour",
          title: "Reservations by Hour",
          data: Object.entries(hourBuckets)
            .sort((a, b) => Number(a[0]) - Number(b[0]))
            .map(([h, c]) => ({ name: `${h}:00`, value: c })),
        },
      ],
    };
  }
  ...
  return {
    ...
    message: `${reservations.length} reservation(s) found.${reservations
      .slice(0, 5)
      .map(
        (r: any) =>
          ` ${r.guestName || "Guest"} (${r.partySize}) - ${new Date(r.date).toLocaleString()}`,
      )
      .join(";")}`,
  };
}
...
export async function manageTables(userId: string, params: any) {
  const ctx = await resolveDalContext(userId);
  const db = getCrmDb(ctx);
  const tables = await (db as any).restaurantTable
    .findMany({
      where: { userId: ctx.userId },
  ...
  return {
    ...
    statistics: {
      ...
      totalCapacity: tables.reduce(
        (s: number, t: any) => s + (t.capacity || 0),
        0,
      ),
    ...
    message: `${tables.length} tables: ${available.length} available, ${occupied.length} occupied. Total capacity: ${tables.reduce((s: number, t: any) => s + (t.capacity || 0), 0)} seats.`,
  };
}
...
export async function getTeamPerformance(userId: string, params: any) {
  ...
  const [members, tasks, deals] = await Promise.all([
    ...
    db.deal.findMany({
      where: { userId: ctx.userId, actualCloseDate: { gte: since } },
      select: { assignedToId: true, value: true },
    }),
  ]);
  const perf = members.map((m: any) => ({
    name: m.user?.name || m.user?.email || "Unknown",
    role: m.role,
    tasksCompleted: tasks.filter((t: any) => t.assignedToId === m.id).length,
    dealsClosed: deals.filter((d: any) => d.assignedToId === m.id).length,
    revenue: deals
      .filter((d: any) => d.assignedToId === m.id)
      .reduce((s: number, d: any) => s + (d.value || 0), 0),
  }));
  return {
    success: true,
    triggerVisualization: true,
    statistics: { teamSize: members.length, performance: perf },
    message: `Team of ${members.length} (last ${days}d): ${perf.map((p: any) => `${p.name}: ${p.tasksCompleted} tasks, ${p.dealsClosed} deals, $${p.revenue.toLocaleString()}`).join("; ")}`,
    dynamicCharts: [
      {
        chartType: "bar",
        dimension: "team_tasks",
        title: "Tasks Completed by Team",
        data: perf.map((p: any) => ({ name: p.name, value: p.tasksCompleted })),
      },
    ],
  };
}
...
export async function getAuditLog(userId: string, params: any) {
  ...
  return {
    ...
    message: `${logs.length} recent activities: ${logs
      .slice(0, 5)
      .map(
        (l: any) => `${l.action} (${new Date(l.createdAt).toLocaleString()})`,
      )
      .join("; ")}`,
  };
}
...
export async function checkIntegrations(userId: string) {
  ...
  return {
    success: true,
    statistics: { total: all.length, active: active.length, integrations: all },
    message: `${all.length} integrations: ${active.length} active, ${all.length - active.length} inactive. ${all.map((i) => `${i.name} (${i.status})`).join(", ")}`,
  };
}
...
export async function manageReviews(userId: string, params: any) {
  ...
  const ctx = await resolveDalContext(userId);
  const db = getCrmDb(ctx);
  const reviews = await (db as any).review
    .findMany({
  ...
  if (action === "recent") {
    return {
      ...
      message: `Recent reviews: ${reviews
        .slice(0, 5)
        .map(
          (r: any) =>
            `${r.customerName || "Customer"}: ${r.rating}★${r.comment ? ` - "${r.comment.slice(0, 50)}"` : ""}`,
        )
        .join("; ")}`,
    };
  }
  return {
    ...
    message: `Average rating: ${avg}★ from ${reviews.length} reviews. Distribution: ${Object.entries(
      dist,
    )
      .map(([k, v]) => `${k}★: ${v}`)
      .join(", ")}`,
...
export async function getReferralStats(userId: string) {
  const ctx = await resolveDalContext(userId);
  const db = getCrmDb(ctx);
  const referrals = await (db as any).referral
    .findMany({
  ...
  const totalRewards = referrals.reduce(
    (s: number, r: any) => s + (r.rewardAmount || 0),
    0,
  );
  return {
    success: true,
    ...
    message: `Referral program: ${referrals.length} total, ${converted} converted (${referrals.length > 0 ? Math.round((converted / referrals.length) * 100) : 0}%). Total rewards: $${totalRewards.toLocaleString()}.`,
  };
}
...
export async function getIndustryAnalytics(userId: string, params: any) {
  ...
  if (industry === "REAL_ESTATE") {
    const [props, fsbo, cma, presentations, topCity] = await Promise.all([
      (db as any).rEProperty
        .count({ where: { userId: ctx.userId } })
        .catch(() => 0),
      (db as any).rEFSBOListing
        .count({ where: { userId: ctx.userId } })
        .catch(() => 0),
      (db as any).rECMAReport
        .count({ where: { userId: ctx.userId } })
        .catch(() => 0),
      (db as any).rEListingPresentation
        .count({ where: { userId: ctx.userId } })
        .catch(() => 0),
      (db as any).rEProperty
        .findFirst({
  ...
  if (industry === "RESTAURANT" || (industry as string) === "FOOD_SERVICE") {
    const [reservations, tables] = await Promise.all([
      (db as any).reservation
        .count({ where: { userId: ctx.userId } })
        .catch(() => 0),
      (db as any).restaurantTable
        .count({ where: { userId: ctx.userId } })
        .catch(() => 0),
    ]);
    return {
      success: true,
      statistics: { industry, reservations, tables },
      message: `Restaurant: ${reservations} reservations, ${tables} tables.`,
    };
  }

  if (industry === "SPORTS_CLUB" || (industry as string) === "YOUTH_SPORTS") {
    const [regs, programs, teams] = await Promise.all([
      (db as any).clubOSRegistration
        .count({ where: { program: { userId: ctx.userId } } })
        .catch(() => 0),
      (db as any).clubOSProgram
        .count({ where: { userId: ctx.userId } })
        .catch(() => 0),
      (db as any).clubOSTeam
        .count({ where: { division: { userId: ctx.userId } } })
        .catch(() => 0),
    ]);
    return {
      success: true,
      statistics: { industry, registrations: regs, programs, teams },
      message: `ClubOS: ${regs} registrations, ${programs} programs, ${teams} teams.`,
    };
  }
  ...
  return {
    success: true,
    statistics: { industry },
    message: `Industry: ${industry}. Use get_statistics for general CRM analytics.`,
  };
}
...
export async function getBusinessScore(userId: string) {
  ...
  const [leads, deals, tasks, payments, reviews] = await Promise.all([
    ...
    db.deal.findMany({
      where: { userId: ctx.userId },
      select: { value: true, actualCloseDate: true, stageId: true },
    }),
    db.task.findMany({
      where: { userId: ctx.userId },
      select: { status: true },
    ...
    db.payment.findMany({
      where: { userId: ctx.userId, status: "SUCCEEDED" },
      select: { amount: true },
    }),
    (db as any).review
      .findMany({
  ...
  const scores = {
    ...
    pipelineVelocity: Math.min(
      deals.length > 0 ? Math.round((closedDeals / deals.length) * 100) : 0,
      100,
    ),
    revenueTrend:
      predictions.growthTrend === "accelerating"
        ? 90
        : predictions.growthTrend === "steady"
          ? 70
          : predictions.growthTrend === "declining"
            ? 30
            : 50,
    teamProductivity:
      tasks.length > 0 ? Math.round((completedTasks / tasks.length) * 100) : 50,
    customerSatisfaction: Math.round((avgRating / 5) * 100),
    operationalEfficiency: Math.min(
      Math.round(
        ((completedTasks + closedDeals) /
          Math.max(tasks.length + deals.length, 1)) *
          100,
      ),
      100,
    ),
  ...
  return {
    ...
    statistics: {
      ...
      breakdown: scores,
      growthTrend: predictions.growthTrend,
      criticalInsights: highInsights.length,
      topActions: highInsights
        .slice(0, 3)
...
export async function getCostOptimization(userId: string) {
  ...
  const [voiceUsage, smsCampaigns, payments, subscription] = await Promise.all([
    (db as any).voiceUsage
      .findMany({
        where: { userId: ctx.userId },
        select: { minutes: true, cost: true, agentType: true },
        orderBy: { createdAt: "desc" },
        take: 100,
      })
      .catch(() => []),
    db.smsCampaign
      .findMany({
        where: { userId: ctx.userId },
        select: { totalSent: true, totalReplied: true },
      })
      .catch(() => []),
    db.payment.findMany({
      where: { userId: ctx.userId, status: "SUCCEEDED" },
  ...
  const voiceCost = voiceUsage.reduce(
    (s: number, u: any) => s + (u.cost || 0),
    0,
  );
  const voiceMin = voiceUsage.reduce(
    (s: number, u: any) => s + (u.minutes || 0),
    0,
  );
  if (voiceCost > 100)
    suggestions.push(
      `Voice AI: $${voiceCost.toFixed(0)} spent on ${Math.round(voiceMin)} min. Consider optimizing agent prompts to reduce call duration.`,
    );

  const totalSms = smsCampaigns.reduce(
    (s: number, c: any) => s + (c.totalSent || 0),
    0,
  );
  const totalReplied = smsCampaigns.reduce(
    (s: number, c: any) => s + (c.totalReplied || 0),
    0,
  );
  const smsReplyRate = totalSms > 0 ? (totalReplied / totalSms) * 100 : 0;
  if (smsReplyRate < 15 && totalSms > 50)
    suggestions.push(
      `SMS reply rate is ${smsReplyRate.toFixed(1)}% — consider shifting budget to email drip campaigns.`,
    );

  const avgPayment =
    payments.length > 0
      ? payments.reduce((s, p) => s + (p.amount || 0), 0) / payments.length
      : 0;
  if (avgPayment < 1000 && payments.length > 20)
    suggestions.push(
      `Average transaction is $${(avgPayment / 100).toFixed(0)} — consider upselling or bundling products.`,
    );

  if (suggestions.length === 0)
    suggestions.push(
      "Your spending patterns look efficient! No major cost optimization opportunities detected.",
    );
  ...
  return {
    ...
    statistics: {
      voiceCost,
      smsReplyRate: Math.round(smsReplyRate),
      subscriptionCost: (subscription as any)?.amount || 0,
      suggestions,
    },
    message: `Cost Analysis: ${suggestions.join(" ")}`,
  };
}
