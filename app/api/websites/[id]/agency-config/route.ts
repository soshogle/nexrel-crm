...
export async function GET(
  ...
  try {
    ...
    if (previewToken && expectedSecret) {
      try {
        ...
        if (decoded.websiteId === websiteId && decoded.config) {
          ...
          const navConfig =
            (draft.navConfig as Record<string, unknown>) || DEFAULT_NAV_CONFIG;
          const pageLabels =
            (draft.pageLabels as Record<string, string>) || DEFAULT_PAGE_LABELS;
          const crmOrigin =
            request.nextUrl?.origin ||
            process.env.NEXREL_PUBLIC_URL ||
            process.env.NEXREL_CRM_URL ||
            "";
          const mapsScriptUrl =
          ...
          const config = {
            brokerName:
              (draft.brokerName as string) || "Real Estate Professional",
            ...
            tagline:
              (draft.tagline as string) || "Your trusted real estate partner",
            ...
            fullAddress:
              (draft.fullAddress as string) || (draft.address as string) || "",
    ...
    const navConfig =
      storedNav && Object.keys(storedNav).length > 0
    ...
    const pageLabels =
      storedLabels && Object.keys(storedLabels).length > 0
