...
} from "@/lib/listing-enrichment/google-search";
...
export async function POST(
  ...
  try {
    ...
    const [crmProperties, crmRentals] = await Promise.all([
      resolved.db.rEProperty.findMany({
        where: {
          userId,
      ...
      resolved.db.rERentalListing.findMany({
        where: {
          userId,
          listingStatus: "ACTIVE",
          OR: isMlsLike
    ...
    for (const prop of crmProperties) {
      ...
      const syncInput: SyncListingInput = {
        ...
        photos: enriched?.galleryImages?.length
    ...
    for (const rental of crmRentals) {
      const photos = Array.isArray((rental as any).photos)
        ? ((rental as any).photos as string[])
        : [];

      let enriched: EnrichedData | null = null;
      if (rental.mlsNumber) {
        enriched = await safeEnrich(
          rental.mlsNumber,
          rental.city,
          rental.address,
        );
    ...
    // Create in CRM
    try {
      await resolved.db.rEProperty.create({
        data: {
          userId,
          address,
          city,
          ...
          sqft: enriched.area
            ? parseInt(String(enriched.area).replace(/[^0-9]/g, ""), 10) || null
            : null,
          lotSize: enriched.lotArea
            ? parseInt(String(enriched.lotArea).replace(/[^0-9]/g, ""), 10) ||
              null
            : null,
    ...
    const syncInput: SyncListingInput = {
      ...
      photos:
        enriched.galleryImages ||
        (enriched.mainImageUrl ? [enriched.mainImageUrl] : null),
      description: enriched.description || null,
      features: [
        ...(enriched.features?.amenities || []),
        ...(enriched.features?.proximity || []),
...
function mergeEnrichedData(a: EnrichedData, b: EnrichedData): EnrichedData {
  return {
    ...
    galleryImages:
      (b.galleryImages?.length ?? 0) > (a.galleryImages?.length ?? 0)
        ? b.galleryImages
        : a.galleryImages,
...
function mergeFeatures(
  ...
  return {
    ...
    inclusions: [
      ...new Set([...(a.inclusions || []), ...(b.inclusions || [])]),
    ],
  };
}
