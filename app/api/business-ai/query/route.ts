...
export async function POST(request: NextRequest) {
  try {
    ...
    const healthScore =
      businessAnalyticsEngine.calculateHealthScore(businessData);

    // Generate predictions
    const predictions =
      businessAnalyticsEngine.generatePredictions(businessData);
  ...
  switch (metric) {
    ...
    case "products":
      return {
        ...
        chartType: "pie",
        data: {
          labels: businessData.products.topSelling.map(
            (p: any) => p.productName,
          ),
          datasets: [
            {
