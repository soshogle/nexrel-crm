...
async function executeSMS(
  ...
  const actionConfig = task.actionConfig as any;
  const message = actionConfig?.smsMessage || actionConfig?.message || task.description || 'Hello from your team';
...
async function executeEmail(
  ...
  const actionConfig = task.actionConfig as any;
  const subject = actionConfig?.emailSubject || actionConfig?.subject || task.name || 'Message from your team';
