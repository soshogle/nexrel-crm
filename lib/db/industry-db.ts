...
function getIndustryDbUrl(industry: IndustryDbKey | string): string {
  ...
  if (envVar && process.env[envVar]) {
    return normalizeDbUrl(process.env[envVar]);
  }
  return normalizeDbUrl(process.env.DATABASE_URL);
}
 ...
 */
export function getIndustryDb(
  industry: IndustryDbKey | string | null,
): PrismaClient {
  const key = industry || "default";
  let client = industryClients.get(key);
  if (!client) {
    const url = industry
      ? getIndustryDbUrl(industry)
      : normalizeDbUrl(process.env.DATABASE_URL);
    if (!url) {
      throw new Error(
        `Missing database URL for industry client: ${String(industry || "default")}`,
      );
    }
