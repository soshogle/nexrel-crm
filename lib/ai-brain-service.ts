...
export class AIBrainService {
  ...
  async generateGeneralInsights(userId: string): Promise<GeneralInsight[]> {
    ...
    if (recentLeads.length > previousWeekLeads.length * 1.2) {
      insights.push({
        ...
        description: `Your lead generation has increased by ${Math.round(
          ((recentLeads.length - previousWeekLeads.length) /
    ...
    const leadsWithoutNotes = leads.filter(
      (l) => !l.notes || l.notes.length === 0,
    );
    ...
    try {
      ...
      const aiInsights = await this.generateAIInsights({
        ...
        recentWins: leads.filter(
          (l) =>
            l.status === "CONVERTED" &&
            l.updatedAt > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
        ).length,
...
Respond ONLY with valid JSON array, no markdown.`;
  ...
  async generatePredictiveAnalytics(
    ...
    const leadGrowthRate =
      previous30DaysLeads.length > 0
        ? (last30DaysLeads.length - previous30DaysLeads.length) /
          previous30DaysLeads.length
        : 0;

    // Predict next week
    const avgLeadsPerWeek = last30DaysLeads.length / 4;
    const nextWeekLeads = Math.round(
      avgLeadsPerWeek * (1 + leadGrowthRate * 0.25),
    );
    ...
    const avgDealValue =
      last30DaysDeals.length > 0
        ? last30DaysDeals.reduce((sum, d) => sum + (d.value || 0), 0) /
          last30DaysDeals.length
        : 5000;
    const nextWeekRevenue = nextWeekConversions * avgDealValue;

    // Determine growth trend
    let growthTrend: "accelerating" | "steady" | "declining" | "volatile" =
      "steady";
    ...
    const result: PredictiveAnalytics = {
      ...
      nextMonthForecast: {
        newLeads: {
          predicted: Math.round(nextWeekLeads * 4.3),
          confidence: 55,
        },
        dealConversions: {
          predicted: Math.round(nextWeekConversions * 4.3),
          confidence: 50,
        },
        revenue: {
  ...
  private async detectSeasonalPatterns(
    ...
    const entries = Object.entries(monthBuckets).sort(
      (a, b) => Number(b[1]) - Number(a[1]),
    );
    if (entries.length >= 3) {
      patterns.push(
        `Best closing months: ${monthNames[Number(entries[0][0])]}, ${monthNames[Number(entries[1][0])]}`,
      );
    }

    return patterns.length > 0
      ? patterns
      : ["Insufficient historical data for seasonal patterns"];
  }
  ...
  async generateWorkflowRecommendations(
    ...
    if (newLeads.length > 5) {
      recommendations.push({
        ...
        description:
          "Automatically create a follow-up task 24 hours after a new lead is created",
    ...
    if (unassignedTasks.length > 5) {
      recommendations.push({
        ...
        description:
          "Automatically assign tasks to team members based on workload and expertise",
    ...
    if (closedDeals.length > 10) {
      recommendations.push({
        ...
        expectedImpact:
          "Learn from successes and failures; improve sales process",
        automatable: true,
        priority: "medium",
      });
    }
   ...
   */
  async detectRealtimePatterns(userId: string): Promise<GeneralInsight[]> {
    const insights: GeneralInsight[] = [];
    const summary = await crmEvents.getDurableEventSummary(userId, 60);
    ...
    if (leadsCreated >= 5) {
      insights.push({
        ...
        actionable: true,
        suggestedActions: [
          "Prioritize follow-ups on newest leads",
          "Check which source is generating the surge",
        ],
        timestamp: new Date(),
      });
    }
    ...
    if (campaignsSent >= 1) {
      insights.push({
        ...
        actionable: true,
        suggestedActions: [
          "Check open rates in 1-2 hours",
          "Prepare follow-up for non-openers",
        ],
        timestamp: new Date(),
      });
    }
