...
const PROCEDURE_COSTS: Record<
  string,
  {
    now: number;
    escalated6mo: number;
    escalated12mo: number;
    description: string;
  }
> = {
  filling: {
    ...
    escalated6mo: 450,
    escalated12mo: 2800,
    description: "Filling → Root Canal + Crown",
  },
  caries: {
    now: 250,
    escalated6mo: 450,
    escalated12mo: 2800,
    description: "Caries Treatment → Root Canal + Crown",
  },
  crown: {
    now: 1200,
    escalated6mo: 1500,
    escalated12mo: 3500,
    description: "Crown → Extraction + Implant",
  },
  root_canal: {
    now: 1500,
    escalated6mo: 1800,
    escalated12mo: 4200,
    description: "Root Canal → Extraction + Implant",
  },
  scaling: {
    now: 380,
    escalated6mo: 1200,
    escalated12mo: 2200,
    description: "Scaling → Surgical Perio",
  },
  perio_moderate: {
    now: 600,
    escalated6mo: 1800,
    escalated12mo: 3500,
    description: "Deep Cleaning → Gum Surgery",
  },
  perio_severe: {
    now: 1800,
    escalated6mo: 3500,
    escalated12mo: 5500,
    description: "Perio Surgery → Extraction + Bone Graft + Implant",
  },
  extraction: {
    now: 350,
    escalated6mo: 350,
  ...
  implant: {
    ...
    escalated6mo: 5000,
    escalated12mo: 6500,
    description: "Implant + Bone Graft",
  },
};
...
function getCostsForTooth(
  ...
  if (currentCondition === "caries" || currentCondition === "filling") {
    ...
    return {
      now: base.now,
      at6mo: base.escalated6mo,
      at12mo: base.escalated12mo,
    };
  }
...
function computeSiteRate(
  ...
  const popRate =
    (currentBop ? POP_PD_RATE_WITH_BOP : POP_PD_RATE_PER_MONTH) *
    riskMultiplier;
...
function linearRegressionSlope(
  points: Array<{ date: Date; pd: number }>,
): number {
  ...
  const xs = points.map(
    (p) => (p.date.getTime() - t0) / (1000 * 60 * 60 * 24 * 30.44),
  ); // months
...
function extractAiInsights(xrayFindings: XrayFinding[]): {
  ...
  for (const xray of xrayFindings) {
    const findingsText = typeof xray.findings === "string" ? xray.findings : "";
    const recommendationsText =
      typeof xray.recommendations === "string" ? xray.recommendations : "";

    if (findingsText) {
      summary.push(`[${xray.xrayType}] ${findingsText}`);
    }
    if (recommendationsText) {
      summary.push(`Recommendations: ${recommendationsText}`);
    }
    ...
    for (const toothNum of xray.teethIncluded) {
      ...
      const patterns = [
        {
          regex: new RegExp(`#?${toothNum}[^0-9].*?(caries|cavity|decay)`, "i"),
          msg: `Caries detected on x-ray (${xray.xrayType})`,
        },
        {
          regex: new RegExp(
            `#?${toothNum}[^0-9].*?(bone loss|bone level)`,
            "i",
          ),
          msg: `Bone loss noted on x-ray (${xray.xrayType})`,
        },
        {
          regex: new RegExp(
            `#?${toothNum}[^0-9].*?(periapical|abscess|lesion)`,
            "i",
          ),
          msg: `Periapical finding on x-ray (${xray.xrayType})`,
        },
        {
          regex: new RegExp(`#?${toothNum}[^0-9].*?(fracture|crack)`, "i"),
          msg: `Possible fracture noted on x-ray (${xray.xrayType})`,
        },
        {
          regex: new RegExp(`#?${toothNum}[^0-9].*?(fill|restor)`, "i"),
          msg: `Restoration noted on x-ray (${xray.xrayType})`,
        },
      ];

      for (const p of patterns) {
        if (p.regex.test(findingsText)) {
    ...
    if (plan.status === "CANCELLED" || !plan.patientConsent) {
      for (const proc of plan.procedures || []) {
        if (proc.toothNumber) {
          ...
          let consequence =
            "Condition may have progressed since recommendation was declined.";
          if (monthsAgo > 6) {
...
function buildTrendData(
  ...
  points.push({
    date: in6.toISOString(),
    pd: projectedPd6mo,
    type: "projected",
  });
  points.push({
    date: in12.toISOString(),
    pd: projectedPd12mo,
    type: "projected",
  });
...
export function computeTrajectory(
  ...
  const latestOdontogram =
    odontogramSnapshots.length > 0
      ? [...odontogramSnapshots].sort(
          (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime(),
        )[0]
      : null;

  // Confidence based on data availability
  const historySpanMonths =
    sortedExams.length >= 2
      ? monthsDiff(
          new Date(sortedExams[0].date),
          new Date(sortedExams[sortedExams.length - 1].date),
        )
      : 0;
  ...
  for (let t = 1; t <= 32; t++) {
    ...
    if (currentCondition === "missing" || currentCondition === "extraction") {
      teeth[toothNum] = createMissingToothProjection(
        toothNum,
        currentCondition,
      );
      continue;
    ...
    for (const site of SITES) {
      ...
      const { rate, source } = computeSiteRate(
        toothNum,
        site,
        sortedExams,
        riskMultiplier,
    ...
    const worstCurrent = Math.max(
      ...SITES.map((s) => siteProjections[s].currentPd),
    );
    const worst6 = Math.max(
      ...SITES.map((s) => siteProjections[s].projectedPd6mo),
    );
    const worst12 = Math.max(
      ...SITES.map((s) => siteProjections[s].projectedPd12mo),
    );
    ...
    const mobilityRisk12: 0 | 1 | 2 | 3 =
      worst12 >= 9 ? 3 : worst12 >= 7 ? 2 : worst12 >= 5 ? 1 : 0;
    ...
    const projCondition6 = projectCondition(
      currentCondition,
      conditionAge,
      6,
      worstCurrent,
    ...
    const projCondition12 = projectCondition(
      currentCondition,
      conditionAge,
      12,
      worstCurrent,
    ...
    else if (worst12 >= 7 || projCondition12 !== currentCondition)
      riskLevel = "high";
    else if (worstCurrent >= 5 || (worst6 >= 5 && hasBopAnywhere))
      riskLevel = "moderate";

    // Alerts
    const alerts: string[] = [];
    if (hasBopAnywhere)
      alerts.push("Active bleeding on probing — inflammation present");
    if (worst6 >= 5)
      alerts.push(`Pocket depth projected to reach ${worst6}mm in 6 months`);
    if (worst12 >= 7)
      alerts.push(`Severe pocket depth projected at 12 months (${worst12}mm)`);
    if (extractionRisk)
      alerts.push("Tooth at risk of extraction within 12 months");
    if (projCondition6 !== currentCondition) {
      alerts.push(
        `Condition may progress from ${currentCondition} to ${projCondition6} by 6 months`,
      );
    }
    if (
      projCondition12 !== currentCondition &&
      projCondition12 !== projCondition6
    ) {
    ...
    const declinedForTooth = declinedTreatments.filter(
      (d) => d.toothNumber === toothNum,
    );
    for (const d of declinedForTooth) {
      alerts.push(
        `Previously recommended: ${d.procedure}. ${d.consequenceNow}`,
      );
    }

    // Costs
    const costs = getCostsForTooth(
      currentCondition,
      worstCurrent,
      worst6,
      worst12,
    ...
    const worstSite = SITES.reduce(
      (best, s) =>
        siteProjections[s].currentPd > siteProjections[best].currentPd
          ? s
          : best,
      SITES[0],
    );
  ...
  const projectedExtractions = allTeeth.filter(
    (t) => t.extractionRisk12mo,
  ).length;
  ...
  else if (atRisk.filter((t) => t.riskLevel === "high").length > 2)
    overallRisk = "high";
  else if (atRisk.length > 0) overallRisk = "moderate";

  // Top alerts — deduplicate and take most important
  const allAlerts = allTeeth.flatMap((t) =>
    t.alerts.map((a) => `Tooth #${t.toothNumber}: ${a}`),
  );
      ...
      const priority = (s: string) =>
        s.includes("extraction")
          ...
          : s.includes("critical")
            ...
            : s.includes("severe")
              ? 2
              : s.includes("Previously recommended")
                ? 3
                : s.includes("progress")
                  ? 4
                  : 5;
...
function createMissingToothProjection(
  toothNum: string,
  condition: ToothCondition,
): ToothProjection {
  const emptySite: SiteProjection = {
    ...
    projectedPd12mo: 0,
    currentBop: false,
    projectedBop6mo: false,
    projectedBop12mo: false,
    ratePerMonth: 0,
