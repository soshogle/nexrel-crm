...
export const smsCampaignService = {
  async findMany(ctx: DalContext, options?: { take?: number; skip?: number; where?: any; include?: any; orderBy?: any }) {
    ...
    return getCrmDb(ctx).smsCampaign.findMany({
      where,
  ...
  async findUnique(ctx: DalContext, id: string, include?: any) {
    return getCrmDb(ctx).smsCampaign.findFirst({ where: { id, userId: ctx.userId }, include } as any);
  },
  async create(ctx: DalContext, data: any) {
    return getCrmDb(ctx).smsCampaign.create({ data: { ...data, userId: ctx.userId } } as any);
  },
  async update(ctx: DalContext, id: string, data: any) {
    return getCrmDb(ctx).smsCampaign.update({ where: { id, userId: ctx.userId }, data } as any);
  },
  async delete(ctx: DalContext, id: string) {
    return getCrmDb(ctx).smsCampaign.delete({ where: { id, userId: ctx.userId } } as any);
  },
  async count(ctx: DalContext, where?: any) {
    return getCrmDb(ctx).smsCampaign.count({ where: { userId: ctx.userId, ...where } } as any);
  },
};
