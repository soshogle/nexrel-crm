...
async function sendSMSNotification(phone: string, data: HITLNotificationData): Promise<void> {
  try {
    ...
    const approvalUrl = `${process.env.NEXTAUTH_URL || process.env.APP_URL || 'https://app.nexrel.com'}/dashboard/real-estate/workflows`;
      ...
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background: #f9fafb; padding: 30px; border: 2px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">
          ...
          <div style="margin-top: 30px; text-align: center;">
            <a href="${approvalUrl}" style="background: linear-gradient(135deg, #8b5cf6 0%, #a78bfa 100%); color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">
              View & Approve
            </a>
          </div>
        </div>
